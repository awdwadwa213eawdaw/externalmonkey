...
local function shutdown()
	...
	players.PlayerAdded:connect(function(p)
		wait()
		p:Kick('Please Rejoin! :)')
	end)
	for _, p in pairs(players:GetChildren()) do
		pcall(function() p:Kick('Please Rejoin! :)') end)
	end
end
...
local function globalNotificationReceived(n)
	...
	elseif n.kind == 'WeatherUpdate' then
		_f.Connection:postAll('weatherChange', {
			End={
				{''},
				_f.currentWeather
			},
			...
			StartNotif={
				...
				regionName='Forced',
				weatherKind=n.startWeather
			}})
		_f.currentWeather = n.startWeather
	end
end
...
---------------------------------------------------------------------------------------------
local stores = _f.FirebaseService

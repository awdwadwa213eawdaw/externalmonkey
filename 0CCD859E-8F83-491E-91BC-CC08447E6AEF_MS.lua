return function(_p)
	...
	local pc_bgs = {
		-- types
		6789241623, -- grass    (old 297119555)
		6789242096, -- fire
		6789242536, -- water
		6789243004, -- electric
		6789243806, -- ground
		...
		6789251959, -- cave
		6789252757, -- iceberg
		6789253442, -- waterfall
		6789253848, -- city
		6789254527, -- galaxy
		6789255674, -- galaxy 2
		7822783014, -- mega charizards
	...
	local function onOptionClicked(option)
		if option == 'Summary' then
			...
			p = _p.Connection:get('PDS', 'cPC', 'getSummary', p[1])
			if not p then enabled = true return end
		...
		elseif option == 'Nickname' then -- TODO: prevent nicknaming Eggs?
			...
			_p.Connection:get('PDS', 'cPC', 'nickname', p[1], name)
			enabled = true
		...
		elseif option == 'Release' then
			...
			enabled = false
			local pName = _p.Connection:get('PDS', 'cPC', 'getItem', p[1])
			if pName then
				local chat = _p.NPCChat
				...
				local rf = _p.RoundedFrame:new {
					...
					MouseButton1Click = function() -- an option was clicked
						...
						if optionNum == 1 then--RENAME
							...
							name = _p.Connection:get('PDS', 'cPC', 'nameBox', boxNum, name) or ('Box '..boxNum)
							entryBox:Remove()
						...
						elseif optionNum == 2 then--WALLPAPER
							...
							if wNum then
								...
								spawn(function() _p.Connection:get('PDS', 'cPC', 'setWPaper', boxNum, wNum) end)
	...
	function pc_ui:bootUp(pcModel)
		...
		Utilities.fastSpawn(function()
			pc = _p.Connection:get('PDS', 'openPC')
		end)
		--// startup animation //--
			...
			local function createRow()
				...
				local function makeStrip()
					local strip = create 'ImageLabel' {
						...
						ImageRectOffset = Vector2.new(0, 30),
						SizeConstraint = Enum.SizeConstraint.RelativeYY,
					...
					if r == 5 and _p.DataManager.currentChunk.id == 'chunk3' and not _p.PlayerData.completedEvents.PCPorygonEncountered then -- porygon button
						create 'ImageLabel' {
							...
							ImageRectOffset = Vector2.new(4011, 30*r),
		...
		do
			...
			local party = create 'ImageButton' {
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://13051200575', -- PC Team Container
				Size = UDim2.new(1.0, 0, 1.0, 0),
		...
		local done = false
		Utilities.fastSpawn(function()
			if not _p.Connection:get('PDS', 'closePC', pc.id, pc.ch) then
	...
	_p.Connection:bindEvent('PCPassPurchased', function()
		if not pcOn then

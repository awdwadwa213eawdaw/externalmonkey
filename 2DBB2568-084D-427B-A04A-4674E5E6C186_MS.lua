return function(_p)
	...
	function Chaining:doQueuedUpdates()
		...
		for _, tag in pairs(self.queue) do
			...
			elseif tag == 'doChanceUpdate' then
				shinyChance, roamChance, haChance, maxIvs = _p.Connection:get('PDS', 'getChainInfo')
			end
		end
		self.queue = {}
	end
	...
	function Chaining:updateChain()
		--if not _p.Connection:get('PDS', 'hasIngredient', 'meteorite', 5) then
		--	return
		--end
		self.queue = {}
		if not self.gui then
	...
	function Chaining:updateChainIcon(icon)
		...
		if self.currentDisplayIcon then
			self.currentDisplayIcon:Remove()
		end
	...
	_p.Connection:bindEvent("updateChain", function(info)

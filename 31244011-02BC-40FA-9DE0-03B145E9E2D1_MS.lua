return function(_p)--local _p = require(script.Parent.Parent)--game:GetService('ReplicatedStorage').Plugins)
	...
	local function onButtonClicked(n)
		...
		_p.Connection:post('PDS', 'purchaseRoPower', group, level)
	end

	_p.Connection:bindEvent('rpActivate', function(group, level, duration)
	...
	function powers:open()
		...
		if not gui then
			...
			gui = create 'ImageLabel' { -- 650x400 5597809907
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://6604653604', --342679586
				...
				create 'Frame' {
					...
					create 'ImageLabel' {
						BackgroundTransparency = 1.0,
						Image = 'rbxassetid://6604655277',--383165720
			...
			local button3 = create 'ImageButton' { -- 150x100
				BackgroundTransparency = 1.0,
				Image = 'rbxassetid://6604659383',--290486212',
...
--[[		
		...
		local status
		Utilities.fastSpawn(function()
			status = _p.Connection:get('PDS', 'roStatus')
			local now = tick()
			for i = 1, 7 do

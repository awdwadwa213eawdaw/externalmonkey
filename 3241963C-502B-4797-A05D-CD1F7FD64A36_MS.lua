...
return function(_p)
	...
	local SpinObj = {}
	function MasterControl:Spin(speed, angle)
		if #SpinObj > 0 then return end
		local char = LocalPlayer.Character
		local head = char.Head
		local BodyVel =  _p.Utilities.Create("BodyAngularVelocity")({
			MaxTorque = Vector3.new(300000, 300000, 300000),
			P = 300,
			Parent = head,
		})
		table.insert(SpinObj, BodyVel)
		local BodyGyro = _p.Utilities.Create("BodyGyro")({
			MaxTorque = Vector3.new(400000, 0, 400000),
			D = 500,
			P = 300,
			Parent = head,
		})
		table.insert(SpinObj, BodyGyro)
		--if angle then
		--	BodyVel.AngularVelocity = angle
		--else
		BodyVel.AngularVelocity = Vector3.new(0,speed,0)
		--end
		return BodyVel, BodyGyro
	end
	function MasterControl:StopSpin()
		if #SpinObj > 0 then
			for i=1, #SpinObj do
				SpinObj[i]:Remove()
			end

...
function BattleGui:animAbility(poke, abilityName)
	...
	local gui = create 'ImageLabel' {
		BackgroundTransparency = 1.0,
		Image = 'rbxassetid://'..({6604463794, 6604466114})[n],
			...
			if name == 'Fight' then
				...
				if name == 'Run' then
					...
					elseif battleKind == 'wild' then
						...
						local escaped = _p.Connection:get('BattleFunction', _p.Battle.currentBattle.battleId, 'tryRun')
...
-- 2 = must
	...
	self.onTargetClicked = function(n)
		...
		if getTokenForPosition(n) > 0 then
			...
			self.inputEvent:fire('move '..moveNum..((gui.mega.selected and ' mega ') or (gui.zmove.selected and ' zmov ') or ' ')..t)
	...
	local fight, ms, zs = main.fight, gui.mega.spriteLabel, gui.zmove.spriteLabel
...
function BattleGui:exitButtonsMain()
	...
	function BattleGui:updateButtonForMove(button, move)
		...
		button.Image = "rbxassetid://7018618985";
	...
	function BattleGui:fightChoices(moveset, rqPokemon, slot, nActive, alreadyChoseMega)
		...
		self.onMoveClicked = function(m)
			...
			if nActive > 1 then
				...
				self.inputEvent:fire('move '..m..((gui.mega.selected and ' mega') or (gui.zmove.selected and ' zmov') or ''))
		...
		if not moves then
			...
			gui.mega = mega
			local zmove = _p.AnimatedSprite:new({
				sheets = {
			...
			s.MouseButton1Click:connect(function()
				...
				if zmove.paused then
					...
					local container = main.container
					local moves, ms, cancel, zs = gui.moves, gui.mega.spriteLabel, gui.cancel, gui.zmove.spriteLabel
			...
			cancel.Visible = true
			function self.moveUpdate()
				--print("move update")

				for i = 1, 4 do
					if zmove.selected then
						local z = zMoves and zMoves[i] ~= 0 and zMoves[i] or nil

						self:updateButtonForZMove(moves[i], moveset[i], z, zMoves)
					else
						-- Update move button
			...
			local ms, zs = mega.spriteLabel, zmove.spriteLabel
			Utilities.Tween(.6, 'easeOutCubic', function(a)
				local o = 1-a
				run.Position = UDim2.new(0.0, 0, 135/130/2, 0) + UDim2.new(0.0, 0, 0.0, (Utilities.gui.AbsoluteSize.Y-container.AbsolutePosition.Y+run.AbsoluteSize.Y)*a)
				bag.Position = UDim2.new(-424/522, 0, 0.0, 0) + UDim2.new(0.0, -(container.AbsolutePosition.X+bag.AbsoluteSize.X)*a, 0.0, 0)
...
--				local partyData = Menu.party.partyData
				...
				elseif pokemonSwitchingOutIsTrapped then
					...
					if pokemonSwitchingOutIsMaybeTrapped then -- OVH  todo: test this
						local battle = _p.Battle.currentBattle
						local t, s, e = _p.Connection:get('BattleFunction', battle.battleId, 'isTrapped', battle.sideId, toSlot)
						isTrapped = t
						local msg = false
...
--	local sd2 = _p.DataManager:getSprite((shiny and '_SHINY' or '')..'_FRONT', p2, p.gender=='F')
	...
	delay(1, function()
		if canceled then return end
			sound = Utilities.loopSound(9933807847)
	end)
...
--		topGui:Remove()
	...
	pcall(function()
		...
		Utilities.sound(9933808921, nil, .5, 10)

		...
		onUse = function(pokemon, battle)
			...
			pcall(function()
				if pokemon:isLead() then
					_f.Connection:post('PDChanged', pokemon.PlayerData.player, 'firstNonEggAbility', pokemon:getAbilityName())
				end
			end)
...
--		onUse = function() end
	...
	['rarecandy'] = {
		...
		onUse = function(poke, battle)
			...
			pcall(function()
				if poke:isLead() then
					_f.Connection:post('PDChanged', PlayerData.player, 'firstNonEggLevel', level)
				end
			end)

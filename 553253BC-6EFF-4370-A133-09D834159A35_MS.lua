...
return {
	...
	gmaxfoamburst = {
		...
		onHit = function(source, target) 
			for _, side in pairs(source.side.foe.active) do
	...
	maxguard = {
		...
		onPrepareHit = function(pokemon) 
			return self:willAct() and self:runEvent('StallMove', pokemon)
		end,
		onHit = function(pokemon) 
			pokemon:addVolatile('stall')
		end,
		condition = {
			...
			onTryHit = function(target, source, move)
				...
				if table.find(bypassesMaxGuard, move.id) then return end

...
local COLOR_PARTNERREADY_ICON  = Color3.fromHSV(1/3, .5, .3)--BrickColor.new('Dark green').Color--
	...
	local ID_CIRCLE = 'rbxassetid://11604162891'
...
function tp:getOrder(battle)
	...
	if is2v2 then
		...
		_p.Connection:bindEvent('2v2TPUS', function(originalPartyIndex, activePartyIndex)
...
--					print(originalPartyIndex, isInActiveParty and tonumber(home.Name) or nil)
					_p.Connection:post('2v2e', 'updateSelection', originalPartyIndex, isInActiveParty and tonumber(home.Name) or nil)
				end
			end
		end
		--
...
--		local pokemon = _p.PlayerData.party[i]
	...
	autoFinish:connect(function()
		...
		if not is2v2 then
			_p.Connection:post('BattleRequest', battle.opponent, {teamPreviewReady = true})
		end
	...
	close:wait()
	if is2v2 then
		_p.Connection:post('2v2e', 'updateSelection', 'ready')
	end
	return order, function()
		_p.Connection:bindEvent('2v2TPUS', nil)

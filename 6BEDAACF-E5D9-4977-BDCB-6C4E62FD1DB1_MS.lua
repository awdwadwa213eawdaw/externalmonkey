return function(_p)--local _p = require(script.Parent.Parent)--game:GetService('ReplicatedStorage').Plugins)
	...
	local BACKGROUND_COLOR = color(5, 5, 5)
	local GRID_COLOR = color(8, 222, 223)
	local ENTRY_THEME_COLOR = Color3.new(0.223529, 0.133333, 0.223529)
	...
	local getIcon; do
		local skip = { 4,   8,   9,  13,  70, 100, 122, 135, 139, 152, 161, 162, 194,
			...
			1023,1024,1025,1104,1105,1106,1107,1108,1109,1110,1111,1112,1113,1114,1115,1116,1117,1118,1119,1120,1121,1122,1123,1124,1125,1126,1127,1128,1129,1130,1131,1132,1135,1136,1137,1138,1139,1140,1141,1142,1143,1144,1145,1146,1147}
	...
	function pokedex:open()
		...
		local dex
		Utilities.fastSpawn(function() dex = _p.Connection:get('PDS', 'getDex') end)
		...
		if not gui then
			...
			local outerBorder = _p.RoundedFrame:new {
				BackgroundColor3 = color(0.0862745, 0.0392157, 0.0862745),
			...
			local innerBorder = _p.RoundedFrame:new {
				BackgroundColor3 = color(8, 222, 223),

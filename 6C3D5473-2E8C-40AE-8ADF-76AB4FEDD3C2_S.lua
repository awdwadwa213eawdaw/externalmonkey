...
--	Utilities.sound(5782801042)
	...
	expEf.BlastRadius = 0
	expEf.RemoveJointRadiusPercent = 0
...
--	Utilities.sound(1577567682)
	nova.Sun.Biggest:Remove()
	nova.Sun.Biggest2:Remove()
	nova.Sun.Biggest3:Remove()
	nova.Sun.Lens:Remove()
	...
	spawn(function()
		...
		Utilities.Tween(1, "easeInSine", function(a)
			nova.EarthRemove.CFrame = nova.EarthRemove.CFrame * CFrame.new(0, 0, 5 * a)
		end)
	end)
	spawn(function()

		Utilities.Tween(1, "easeInSine", function(a)
			nova.EarthRemove2.CFrame = nova.EarthRemove2.CFrame * CFrame.new(0, 0, 5 * a)
		end)
	end)
	delay(1, function()
		spawn(function()
			...
			wait(0.3)
			local dPlanet = game.ReplicatedStorage.Models.Misc["Removeed Planet"]:Clone()
		nova.DarkEarth:Remove()

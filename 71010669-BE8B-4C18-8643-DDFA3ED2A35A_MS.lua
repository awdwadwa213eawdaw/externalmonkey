...
return function(_p)--local _p = require(script.Parent)
	...
	local function onObtainBadgeSound()
		Utilities.sound(10731708541, nil, nil, 10)
	end
	...
	_p.storage.Remote.PickUpManaphyEgg.PlaySound.OnClientEvent:connect(function(eggMain)
		...
		pcall(function()
			...
			Utilities.sound(10731716753, 4, nil, 10, eggMain)
		end)
	end)

	_p.Connection:bindEvent('eggFound', function()
		pcall(function()
	...
	_p.Connection:bindEvent('finishedpuz', function()
		local chunk = _p.DataManager.currentChunk
		...
		spawn(function()  _p.PlayerData:completeEvent('DidRegiPuzzles') end)
	...
	return {
		...
		--Weather
	--[[	['onEnterRegion_Between Dreams'] = function(chunk)
			if not _p.Overworld.Weather.NorthernLights.chunk then 
				spawn(function() 
					delay(2, function()
						_p.Overworld.Weather.NorthernLights:returnHome()
					end)
				end)
				--_p.Weather.NorthenLights:enableNorthenLights(chunk.map:GetBoundingBox())
				--_p.Weather.isAurora = true
			end
		end,]]--
		...
		onLoad_colosseum = function(chunk)
			...
			interact[moveTutor.model] = function() 
				...
				Utilities.fastSpawn(function()
					r = _p.Connection:get('PDS', 'moveTutor')
					done = true
				end)
				...
				local moveTutor = function()
					...
					local mon
					mon, r = _p.Connection:get('PDS', 'makeDecision', r.d, slot)
			...
			interact[bottlecap.model] = function()
				...
				local r, r2 = _p.Connection:get('PDS', 'hasbottlecaps')
				if chat:say(bottlecap, '[y/n]In exchange for Bottle Caps, that is! Wanna try?') then
					if r == 0 and r2 == 0 then
						chat:say(bottlecap, 'You don\'t seem to have any Bottle Caps, come back latter if you get any!')
						return
					end
					spawn(function() _p.Menu:disable() end)
					local a = {
					...
					if useCapType == 'Silver Bottle Cap' then
						...
						if chat:say(bottlecap, '[y/n]Alright, so you want to Hyper Train in '..choice..'?') then
							local ivs = _p.Connection:get('PDS', 'getivs', slot, choice)
							if ivs >= 31 then
							...
							_p.Connection:get('PDS', 'trainpokemon', slot, choice)
							chat:say(bottlecap, '...',
								'... ...',
								'... ... ...'
							)
						...
						local trained = _p.Connection:get('PDS', 'getivs', slot, 'all')
						if chat:say(bottlecap, '[y/n]Alright, so you want to Hyper Train all the stats?') then
							...
							chat:say(bottlecap, 'Alright! Time to do some Hyper Training!')
							_p.Connection:get('PDS', 'trainpokemon', slot, 'all')
							chat:say(bottlecap, '...',
								'... ...',
								'... ... ...'
							)
			...
			interact[sixth.model] = function()
				...
				local moveDeleter = function()
					...
					local name, r = _p.Connection:get('PDS', 'deleteMove', slot)
					if not r then
						sixth:Say('Oh, okay.', 'Well I\'ll be here if you change your mind later.')
						return
					elseif r == 'eg' then
					...
					if not moveslot or not sixth:Say('[y/n]Are you sure you want your ' .. name .. ' to forget ' .. r.moves[moveslot].name .. '?') then
						spawn(function() _p.Connection:get('PDS', 'makeDecision', r.d, nil) end)
						sixth:Say('Oh, okay.', 'Well I\'ll be here if you change your mind later.')
						return
					end
					spawn(function() _p.Connection:get('PDS', 'makeDecision', r.d, moveslot) end)
			...
			interact[srybon.model] = function()
				...
				local done = false
				Utilities.fastSpawn(function()
					r = _p.Connection:get('PDS', 'remindMove')
					done = true
				end)
				...
				local moveReminder = function()
					...
					local name
					name, r = _p.Connection:get('PDS', 'makeDecision', r.d, slot)
					if not r then
						srybon:Say('If you change your mind later, be sure to come back.')
						return
					elseif r == 'eg' then
		...
		onBeforeEnter_lab = function(room)
			local starterData = _p.Connection:get('PDS', 'getStarterData')
			local sds = {}
			...
			if completedEvents.MeetJake then
				...
				local function pickStarter(og)
					if not og and _p.gamemode == 'randomizer' then
						starterData = _p.Connection:get('PDS', 'getStarterData')
					end
				...
				local function setupDevProduct()
					...
					interact[david.model] = function()
						spawn(function() _p.Menu:disable() end)
						local res = (function()
							if not chat:say(david, 'The professor sells these pokemon for 10 R$ each.', '[y/n]Would you like to purchase one?') then return end
							...
							local r = _p.Connection:get('PDS', 'buyStarter', species)
							...
							elseif r == 'to' then
								...
								local msg = _p.Connection:get('PDS', 'makeDecision', r.d, nickname)
				...
				local function setupJakeFight(jake, prof, mom, dad)
					...
					_p.DataManager:preload(10721430579)
		...
		onBeforeEnter_yourhomef1 = function(room)
			...
			if completedEvents.DefeatHoopa and not completedEvents.talktoafter then
				touchEvent('talktoafter', room.model.Trigger, true, function()
					spawn(function() _p.Menu:disable() end)
					MasterControl.WalkEnabled = false
					MasterControl:Stop()
		...
		onDoorFocused_Gate1 = function()
			...
			local music = Utilities.loopSound(10721442103)
		...
		onLoad_chunk2 = function(chunk)
			local function setupForestBattle()
				_p.DataManager:preload(6860109291, 10721453097)
			...
			interact[silverwing.model] = function()
				...
				if completedEvents.getsilverwing then return end
				if _p.Connection:get('PDS', 'has3birds') then
					...
					spawn(function() _p.PlayerData:completeEvent('getsilverwing') end)
		...
		onBeforeEnter_PokeCenter = function(room)
			...
			if npc then
				...
				interact[npc.model] = function()
					...
					if not chat:say(npc, 'Welcome to the Pokemon Center!', '[y/n]Would you like to rest your pokemon?') then
						chat:say(npc, 'Oh alright, I understand. Come back later if you need anything.') spawn(function() _p.Menu:enable() end) return end
					spawn(function() 
						_p.Connection:get('PDS', 'getPartyPokeBalls')
		...
		onBeforeEnter_EeveeHouse = function(room)
			local inGroup = _p.player:IsInGroup(32407347)
		...
		onBeforeEnter_Gym1 = function(room, continueCFrame)
			...
			_p.DataManager:preload(10721460863,
		...
		onBeforeEnter_FriendshipHouse = function(room)
			...
			interact[checker.model] = function()
				local phrase, done
				Utilities.fastSpawn(function()
					phrase = _p.Connection:get('PDS', 'getHappiness')
					done = true
				end)
		...
		onLoad_chunk5 = function(chunk)
			...
			-- VOL ROCK GUY
			if completedEvents.volrock then
				chunk.map.VolRock:Remove()
				chunk.map.VolRock2:Remove()
			end

			local rocknpc = chunk.npcs.ipunchu
			...
			interact[rocknpc.model] = function()
				...
				local hasitems = _p.Connection:get('PDS', 'hasvolitems', 'epineshroom')
				if hasitems.epineshroom then
					...
					if rocknpc:Say('[y/n]Can I have your Epineshroom?') then
						_p.Connection:get('PDS', 'hasvolitems', 'epineshroom', true)
						rocknpc:Say('Thanks!')
						wait(1)
						...
						rocknpc:LookAt(chunk.map.VolRock.Position)
						spawn(function()
		...
		onLoad_chunk6 = function(chunk)
			...
			if completedEvents.DidRegiPuzzles then
				chunk.map.regiblock:Remove()
			end
		end,
		...
		onLoad_chunk7 = function(chunk)
			...
			if completedEvents.GroudonScene then
				...
				_p.DataManager:preload(10721453097, 6860109291, 10723710206, 10723718725,10723724353, 68068592, 317480860) -- admin music, eclipse logo, earthquake, groudon scene music[2], particle, pulse
				...
				touchEvent('GroudonScene', chunk.map.GroudonTrigger, false, function()
					...
					local st = tick()
					local duration = 8
					Utilities.sound(10723710206, .6, nil, 18)
					local sceneMusic = Utilities.sound(10723718725, .3)
					delay(99, function()
						if not sceneMusic then return end
						sceneMusic = Utilities.loopSound(10723724353, .3)
					end)
					while true do
						stepped:wait()
						local et = tick()-st
		...
		onLoad_chunk9 = function(chunk)
			...
			interact[dcm.model] = function()
				if _p.PlayerData.daycareManHasEgg then
					...
					if want then
						local state, done
						Utilities.fastSpawn(function()
							state = _p.Connection:get('PDS', 'takeEgg')
							done = true
						end)
						while not done do wait() end
						...
						if manKeep then
							spawn(function() _p.Connection:post('PDS', 'keepEgg') end)
					...
					local phrase
					Utilities.fastSpawn(function() phrase = _p.Connection:get('PDS', 'getDCPhrase') end)
					dcm:Say('Hey there!')
			...
			if _p.PlayerData.badges[4]  then
				--	_p.Overworld.MarshVoid:connect('MarshadowPortal') --to change
			end
		...
		onBeforeEnter_DayCare = function(room)
			...
			interact[dcl.model] = function()
				...
				spawn(function() _p.Menu:disable() end)
				local dc
				Utilities.fastSpawn(function() dc = _p.Connection:get('PDS', 'getDCInfo') end)
				...
				if option == 1 then
					...
					if slot then
						local r = _p.Connection:get('PDS', 'leaveDCAnimals', slot)
						if r == 'eg' then
				...
				elseif option == 2 then
					...
					if poke then
						...
						if take then
							...
							local done = false
							Utilities.fastSpawn(function()
								_p.Connection:get('PDS', 'takeDCAnimals', pChoice)
								done = true
							end)
		...
		onBeforeEnter_DomeLab = function(room)
			...
			do
				...
				interact[fr.model] = function()
					...
					Utilities.fastSpawn(function()
						hasFossil, hasFossilEgg = _p.Connection:get('PDS', 'hasFossil')
						done = true
					end)
					...
					local function chooseReviveFossil()
						...
						if fossilId then
							...
							Utilities.fastSpawn(function()
								result = _p.Connection:get('PDS', 'reviveFossil', fossilId)
								done = true
							end)
						end
						wait(.6)
...
--[[					if #_p.PlayerData.party == 6 and not _p.PlayerData.pc:hasSpace() then  -- todo?
						fr:Say('Hold on now, you don\'t have any space to store this pokemon if I were to revive it...',
							...
							if fr:Say('[y/n]So, you want to try reviving the ' .. fossilName .. '?') then
								...
								Utilities.fastSpawn(function()
									nn = _p.Connection:get('PDS', 'makeDecision', result[3], true)
									done = true
								end)
								...
								local msg = _p.Connection:get('PDS', 'makeDecision', nn[3], nickname)
								...
								if result and result[3] then spawn(function() _p.Connection:get('PDS', 'makeDecision', result[3], false) end) end
					...
					local function chooseReviveFossilEgg()
						...
						if slot and _p.Connection:get('PDS', 'reviveFossil', slot) then
							fr:Say('Alright, let me see it.')
							fr:Look(Vector3.new(-1, 0, 0))
							machine.Fossil.Transparency = 0
			...
			local function rideUMV(firstTime)
				...
				if firstTime then
					...
					Utilities.fastSpawn(function() fd = _p.Connection:get('PDS', 'dive') end)
				end
			...
			local function buyOrDive()
				...
				local function countBatteries()
					return _p.Connection:get('PDS', 'countBatteries')
				end
		...
		onLoad_chunk11 = function(chunk)
			...
			_p.DataManager:preload(10731716753)
			...
			-- Good rod giver (only obtainable by surfing)
			...
			if completedEvents.LighthouseScene then
				...
				connectLighthouseScene = function()
					...
					touchEvent('LighthouseScene', lhtrigger, false, function()
						...
						_p.DataManager:preload(337448917, 243728104, 10723795043, 10723795043, 337973384, 317480860, 338262406)
						_p.DataManager:preloadModule('AnchoredRig')
						...
						local win = _p.Battle:doTrainerBattle {
							IconId = 6860109291,
							musicId = {10723773194, 10723780906},
							musicVolume = 2,
							PreventMoveAfter = true,
							LeaveCameraScriptable = true,
						...
						local sceneMusic = Utilities.sound(10723795043, .75)--, 35)
						...
						delay(91, function()
							if not sceneMusic then return end
							sceneMusic = Utilities.loopSound(10723795043, .75)
						end)
						local rain
						delay(1, function()
			...
			for _, m in pairs(room.model.Candles:GetChildren()) do
				if m:FindFirstChild('Wick') then
					if math.random(30) == 23 then
						...
						pe.Texture = 'rbxassetid://6604531297'
						...
						chat.silentInteract[m] = function()
							...
							delay(3, function()
								pe.Texture = 'rbxassetid://6604533197'
								pe.Color = ColorSequence.new(
									Color3.new(1, .6, .04),
									Color3.new(1, .92, .06))
							end)
		...
		onBeforeEnter_HMMusicRoom = function(room)
			if _p.PlayerData.rotomEventLevel == 0 then
				...
				local sound = create 'Sound' {
					SoundId = 'rbxassetid://10731743895',
					Volume = 1,
					Looped = true,
					Parent = jukebox.Main,
				}
		...
		onHauntedGameBoyClicked = function(gameboy)
			...
			_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Gameboy,{battleSceneType = 'HauntedMansion', musicId = {10723828869}})
		end,
		--
		...
		onLoad_chunk14 = function(chunk)
			...
			if not completedEvents.Jirachi then
				_p.DataManager:preload(10723837078, 10723844284)
				...
				chatCn = _p.player.Chatted:connect(function(msg)
					...
					if MasterControl.WalkEnabled and not _p.DataManager.isDay and msg:lower():match('^i%s+wish%s+%a') then
						...
						if pos.Y > 0 and pos.Y < 5 and math.sqrt(pos.X^2+pos.Z^2) < 3.5 then
							...
							jirachi.Parent = map
							_p.MusicManager:stackMusic(10723837078, 'Jirachi')
							Connection:post('Doc', 'wish', msg)
							chatCn:disconnect()
							chatCn = nil
							spawn(function() _p.Menu:disable() end)
							MasterControl.WalkEnabled = false
						...
						--]]
							...
							_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Wish,{battleSceneType = 'Jirachi', musicId = 10723844284})
		...
		onLoad_chunk15 = function(chunk)
			...
			-- beekeeper
			local honeyData = _p.Connection:get('PDS', 'getHoneyData')
			...
			interact[beekeeper.model] = function()
				if honeyData.canget then
					...
					honeyData.has = true
					spawn(function() _p.Connection:get('PDS', 'getHoney') end)
		...
		onBeforeEnter_CableCars = function(room) -- can be route 10 OR route 11
			local has = _p.Connection:get('PDS', 'hasSTP')
		...
		onBeforeEnter_DrifloonWindmill = function(room)
			if not _p.Connection:get('PDS', 'isDinWM') then
...
--				blimpEngine.Anchored = true
		...
		onLoad_chunk19 = function(chunk, d) -- ANTHIAN: HOUSING
			...
			if isv then
				local isFull = _p.Connection:get('PDS', 'isTinD')
				if isFull then
		...
		onBeforeEnter_LottoShop = function(room)
			local prizes, triesToday = _p.Connection:get("PDS", "getLottoPrizes")
			...
			interact[hobo.model] = function()
				...
				local function draw()
					...
					spawn(function()
						processing = true
						ticket = _p.Connection:get("PDS", "drawLotto", _p.PlayerData:getEtc())
						triesToday = triesToday + 1
						processing = false
						result = {
							_p.Connection:get("PDS", "getLottoResults")
						}
					end)
					...
					if digitsMatched == 0 then
						...
						if prizeData then
							...
							if prizeIsItem then
								...
								chat.bottom = nil
							elseif prizeData[3] then
								chat.bottom = true
								chat:say(prizeData[1].." was sent to the PC.")
								chat.bottom = nil
							end
						end
					end
		...
		onLoad_chunk20 = function(chunk) -- ANTHIAN: SHOPPIN
			...
			interact[dealer.model] = function()
				local buy = dealer:Say('Hello there, young trainer.',
					...
					'I\'ll turn him over to you for a mere 50 ROBUX.',
				...
				local loadTag = {}
				_p.DataManager:setLoading(loadTag, true)
				local r = _p.Connection:get('PDS', 'buyAshGreninja')
				_p.DataManager:setLoading(loadTag, false)
				chat:manualAdvance()
				if not r then
			...
			local function onClickHoverboard(model)
				...
				if model.Name:sub(1, 6) == 'Basic ' then
					if shopGuy:Say('[y/n]Ah, the '..model.Name..' Board... Would you like to take this one with you?') then
						spawn(function() _p.Connection:get('PDS', 'setHoverboard', model.Name) end)
						pcall(function() csig:fire() end)
					else
						debounce = false
					end
				else
					if _p.Connection:get('PDS', 'ownsHoverboard', model.Name) then
						...
						if shopGuy:Say('[y/n]Ah, '..model.Name..'... Would you like to purchase this one for 10 R$?')
							...
							local loadTag = {}
							_p.DataManager:setLoading(loadTag, true)
							local r = _p.Connection:get('PDS', 'purchaseHoverboard', model.Name, _p.PlayerData:getEtc())
							_p.DataManager:setLoading(loadTag, false)
		...
		onBeforeEnter_RorianBraviary = function(room)
			...
			interact[salesperson.model] = function()
				...
				if c == 1 then
					...
					ok:Remove()
					window:Remove()
					local r = _p.Connection:get('RorianBraviary', 'open')
					if r then
		...
		onBeforeEnter_SushiPlace = function(room)
			...
			interact[salesperson.model] = function()
				...
				moneyFrame:Remove()
				local r = _p.Connection:get('PDS', 'buySushi')
				if r == 'nm' then
		...
		onLoad_chunk22 = function(chunk, data)
			...
			if _p.PlayerData.badges[4] and not completedEvents.DefeatTEinAC then
				_p.DataManager:queueSpritesToCache({'_FRONT', 'Absol'}) -- for the cry
				_p.DataManager:preload(280857070, 10731735388, 507289472, 334858056, 509072758, 509073816, 10723844284)
				...
				spawn(function()
					...
					_p.MusicManager:stackMusic(10723887471, 'Cutscene', .4)
					Utilities.FadeIn(1)
					...
					schedule(changeAnim, 'Grab', .3, function()
						...
						pm.Shirt.ShirtTemplate = 'rbxassetid://8200867736'
						pm.Pants.PantsTemplate = 'rbxassetid://6604541316'
					...
					if d then
						...
						_p.Connection:get('PDS', 'makeDecision', d, slot)
					else
					...
					local st = tick()
					Utilities.sound(10731735388, nil, nil, 15)
					...
					-- Dialga and Palkia
					_p.MusicManager:stackMusic(10723844284, 'Cutscene')
					--				wait(.75)
					...
					local dCry = {id = 9987807545, duration = 1.57, startTime = 2.21}
					local pCry = {id = 9987807545, duration = 1.63, startTime = 4.79}
...
--[[		local lawyer = chunk.npcs.EventLawyer
		...
		onBeforeEnter_Greenhouse = function(room) -- Shaymin Stuff
			...
			local gstate = _p.Connection:get('PDS', 'getGreenhouseState')
			interact[grace.model] = function()
				if gstate.f == 3 then
					...
					if gstate.f == 2 then
						spawn(function() _p.Connection:get('PDS', 'makeDecision', gstate.d) end)
			...
			interact[sanjay.model] = function()
				if completedEvents.GiveEkans then
					...
					if sanjay:Say('I am an aspiring Ekans Charmer.',
						...
						local d
						if tradeslot then
							d = _p.Connection:get('PDS', 'giveEkans', tradeslot)
						end
						if not tradeslot or not d then
							...
							spawn(function() _p.Connection:get('PDS', 'makeDecision', d, trade) end)
							if not trade then
								sanjay:Say('Aw, okay.', 'Let me know if you wanna trade later.')
							else
		...
		onLoad_chunk36 = function(chunk)
			if _p.PlayerData.badges[4]  then
				--			_p.Overworld.MarshVoid:connect('MarshadowPortal') --to change
			end
		end,

		onLoad_chunk99 = function(chunk) 
			if completedEvents.TapuKokoBattle then
				chunk.map.TapuKoko:Remove()
			end
		end,
		...
		onBeforeEnter_PowerPlant = function(room)
			_p.PlayerData.nRotom = _p.Connection:get('PDS', 'hasRTM')
		end,
		...
		onLoad_chunk38 = function(chunk)
			if completedEvents.OpenJDoor then
				...
				_p.PlayerData.hasjkey = _p.Connection:get('PDS', 'hasJKey')
			end
		end,
		...
		onLoad_chunk43 = function(chunk)
			if completedEvents.TERt14 then
				...
				_p.DataManager:preload(10731672046)
				...
				touchEvent(nil, map.CTrigger, false, function()
					...
					_p.MusicManager:stackMusic(10731672046, 'Cutscene')
					wait(.5)
					local speed = .5
					local function schedule(animTrack, kfName, kfTime, func)
						local fired = false
		...
		onLoad_chunk45 = function(chunk)
			...
			spawn(function() _p.DataManager:preload("Sound", 10731727240) end)
			...
			--	local p0, p1, p2, p3 = Vector3.new(2632, 158, 3252), Vector3.new(2516, 138, 3237), Vector3.new(2448, 126, 3120), Vector3.new(2216, 123, 3246)
			touchEvent('EonDuo', chunk.map.CTrigger, true, function()
				...
				Utilities.sound(10731727240, nil, nil, 10)
				local function fly()
				...
				--p0, p1, p2, p3 = p3, 
				--p0, p1, p2, p3 = p3, Vector3.new(2596, 181, 3127), Vector3.new(2372, 204, 2965), Vector3.new(2372, 205, 2965)
					...
					if win then
						...
						spawn(function()
							delay(.5, function()
								door:close(.5)
								Tess:Remove()
								door.disabled = nil -- enable when we continue story
							end)
						end)
					end
					MasterControl.WalkEnabled = true
		...
		onBeforeEnter_HPHouse = function(room)
			local hpguy = room.npcs.hpchecker
			interact[hpguy.model] = function()
				hpguy:Say('If you\'d like, I\'ll check what type of Hidden Power your Pokemon will learn.')
				if hpguy:Say('[y/n]Would you like to find out.') then
					local slot = _p.BattleGui:choosePokemon('Check')
					if not slot then
						hpguy:Say('Ok well feel free to come back anytime!')
						return
					end
					local hptype
					local ya,na = pcall(function() hptype = _p.Connection:get('PDS', 'gethptype', slot) end)
					if na or hptype == nil then hpguy:Say('Hmm, I can\'t seem to check this Pokemon at the moment.') return end
					hpguy:Say('If this Pokemon were to use Hidden Power, the move\'s type would be '..hptype..'.')
				else
					hpguy:Say('Ok well feel free to come back anytime!')
				end
			end
		...
		onLoad_chunk47 = function(chunk)
			local record = chunk.npcs.RecordKeeper
			interact[record.model] = function()
				local dur = _p.Connection:get('PDS', 'getSlopeTime')
				local line
				if dur then
					line = 'Your personal best is '.. string.format("%d:%02d.%02d", math.floor(dur / 45), math.floor(dur % 45), math.floor(dur % 1 * 100)) ..'!'
				end
				record:Say('Hoverboarding down this slope is so fun!',
		...
		onBeforeEnter_SkittyLodge = function(room)
			...
			interact[lady.model] = function()
				if completedEvents.GetSBell then
					...
					chat.bottom = true
					onObtainItemSound()
					spawn(function() _p.PlayerData:completeEvent('GetSBell') end)
		...
		onExit_Gate21 = function()
			...
			--touchEvent('vPortdecca', chunk.map.cTrigger, true, function()
			spawn(function()
				wait(.3)
				spawn(function() _p.Menu:disable() end)
				_p.RunningShoes:disable()
		...
		onLoad_chunk52 = function(chunk)
			...
			interact[captain.model] = function()
				local has = _p.Connection:get('PDS', 'hasTT')
				if not has then
					local has = _p.Connection:get('PDS', 'birdsitem').vt
				end
				if not has then
					local has = _p.Connection:get('PDS', 'birdsitem').ot
				end
				if not has then
					local has = _p.Connection:get('PDS', 'birdsitem').ft
				end
				if not completedEvents.docaptain or not has then
					...
					spawn(function() _p.PlayerData:completeEvent('docaptain') end)
				else
					chat:say(captain, 'Have ye got yerself a ticket.',
						'Aye. Whither shall we sail, eh?'
					)
					local options = {}
					local hasTT = _p.Connection:get('PDS', 'hasTT')
					...
					if #options > 0 then
						...
						if choice == 'Lost Islands' then
							...
							chunk:Remove()
							-- change chunks
							_p.DataManager:loadChunk('chunk70')
						...
						if choice == 'Frigidia Island' then
							...
							chunk:Remove()
							-- change chunks
							_p.DataManager:loadChunk('chunk85')

							--workspace.CurrentCamera.CameraType = Enum.CameraType.Custom
							Utilities.Teleport(CFrame.new(10, 1843.949, 505))
							local elapsed = tick()-startTick
							if elapsed < .5 then
								wait(.5-elapsed)
							end
						...
						if choice == 'Voltridia Island' then
							...
							chunk:Remove()
							-- change chunks
							_p.DataManager:loadChunk('chunk83')

							--workspace.CurrentCamera.CameraType = Enum.CameraType.Custom
							Utilities.Teleport(CFrame.new(-2.807, 150.854, 384.068))
							local elapsed = tick()-startTick
							if elapsed < .5 then
								wait(.5-elapsed)
							end
						...
						if choice == 'Obsidia Island' then
							...
							chunk:Remove()
							-- change chunks
							_p.DataManager:loadChunk('chunk81')

							--workspace.CurrentCamera.CameraType = Enum.CameraType.Custom
							Utilities.Teleport(CFrame.new(186.597, 1105.538, 410))
							local elapsed = tick()-startTick
							if elapsed < .5 then
								wait(.5-elapsed)
							end
		...
		onBeforeEnter_DTA = function(room)
			...
			interact[birds.model] = function()
				local getitemdata = _p.Connection:get('PDS', 'birdsitem')
				local haveall = 0
				...
				if completedEvents.talktobirdsguy then
					if chat:say(birds, 'Ahoy again young sea traveler.',
						...
						if choice == 'Cancel' then
							...
							_p.Connection:get('PDS', 'buybirdsitem', choice)
				...
				_p.PlayerData:completeEvent('talktobirdsguy')
		...
		onBeforeEnter_VolGirlHouse = function(room)
			...
			interact[VolGirl.model] = function()
				...
				if not completedEvents.VolItem1 then
					...
					if VolGirl:Say('[y/n]Do you happen to have a Big Mushroom that I can use?') then
						local hasitems = _p.Connection:get('PDS', 'hasvolitems', 'bigmushroom')
				...
				if not completedEvents.VolItem2 then
					...
					if VolGirl:Say('[y/n]Do you have a Chilan Berry that I can chop up?') then
						local hasitems = _p.Connection:get('PDS', 'hasvolitems', 'chilanberry')
				...
				if not completedEvents.VolItem3 then
					...
					if VolGirl:Say('[y/n]I bet that you\'ve got one that I can have, though?') then
						local hasitems = _p.Connection:get('PDS', 'hasvolitems', 'stardust')
		...
		onBeforeEnter_MagikPond = function(room)
			spawn(function()
				while wait() do
					if _p.PlayerData.hasPondPass then
						room.model.Blocker:Remove()
						break
					end
				end
			end)
			...
			interact[bob.model] = function()
				if _p.PlayerData.hasPondPass then
					...
					bob:Say('Hello there, explorer!',
						...
						'This pond has mystical properties that has allowed the Magikarp to take on new patterns and textures to their skin.',
						...
						'The Pond Pass, which will give you unlimited access to my rare Magikarp pond, will only require you to join the community group listed in the description of the game.'
					)
				end
			end
		end,
		...
		onLoad_chunk74 = function(chunk)
			if completedEvents.GetVolcanion then
				chunk.map.Volcanion:Remove()
			end
		end,
		
		onLoad_chunk84 = function(chunk)
			if completedEvents.getzap then
				chunk.map.ZAP:Remove()
			end
		end,
		onLoad_chunk82 = function(chunk)
			if completedEvents.getmol then
				chunk.map.MOL:Remove()
			end
		end,
		onLoad_chunk86 = function(chunk)
			if completedEvents.getart then
				chunk.map.ART:Remove()
			end
		end,
		onLoad_chunk68 = function(p563)
			if completedEvents.GetMew then
				local l__MewChamber__1959 = p563.map.MewChamber;
				l__MewChamber__1959.Mew:Remove();
				l__MewChamber__1959.TubeInner:Remove();
				local l__Tube__1960 = l__MewChamber__1959.Tube;
				p563.map.Machine["#InanimateInteract"]:Remove();
				local l__CFrame__608 = l__Tube__1960.CFrame;
				Utilities.Tween(0, "easeOutCubic", function(p564)
					l__Tube__1960.CFrame = l__CFrame__608 * CFrame.new(10 * p564, 0, 0);
				end);
				p563.map.Machine.Main.Color = Color3.fromRGB(76, 78, 79);
			end;
		end,

		onBeforeEnter_MewHouse = function(room)
			local l__Guy__609 = room.npcs.Guy;
			interact[l__Guy__609.model] = function()
				l__Guy__609:Say("Upstairs is off-limits.", "We're not hiding anything, now get lost.");
			end;
			if completedEvents.DoMewDoor then
				room.model.PushBarrels:Remove();
				room.model.NoPass.CanCollide = false;
				spawn(function()
					l__Guy__609:Teleport(room.model.GuyWalkTo.CFrame);
					l__Guy__609:LookAt(room.model.Boat.LookAt.CFrame);
				end);
				l__Guy__609.model.Interact.Parent = nil;
				interact[l__Guy__609.model] = function()
					l__Guy__609:Say("Look at what you've done!", "How am I supposed to clean this?");
					l__Guy__609:LookAt(room.model.Boat.LookAt.CFrame);
				end;
				for v1961, v1962 in pairs(room.model:GetChildren()) do
					if v1962:IsA("BasePart") and v1962.Name == "OilSpill" then
						v1962.Transparency = 0;
					end;
				end;
			end
			if completedEvents.OpenMewLab then
				local l__Main__1963 = room.model.Hook.Main;
				local l__CFrame__1964 = l__Main__1963.CFrame;
				local l__Main__1965 = room.model.Crane.Main;
				local l__CFrame__1966 = l__Main__1965.CFrame;
				local l__Main__1967 = room.model.Truck.Main;
				room.model.CraneButton["#InanimateInteract"]:Remove();
				local u610 = l__Main__1963.CFrame;
				local u611 = l__Main__1967.CFrame;
				spawn(function()
					Utilities.Tween(0, nil, function(p566)
						Utilities.MoveModel(l__Main__1963, u610 * CFrame.new(0, 0, 27 * p566));
					end);
					u610 = l__Main__1963.CFrame;
					Utilities.Tween(0, nil, function(p567)
						Utilities.MoveModel(l__Main__1963, u610 * CFrame.new(0, -10.5 * p567, 13.5 * p567));
					end);
					u610 = l__Main__1963.CFrame;
					Utilities.Tween(0, nil, function(p568)
						Utilities.MoveModel(l__Main__1963, u610 * CFrame.new(0, 0, 13.5 * p568));
					end);
					u610 = l__Main__1963.CFrame;
					spawn(function()
						Utilities.Tween(0, nil, function(p569)
							Utilities.MoveModel(l__Main__1967, u611 * CFrame.new(0, 2.625 * p569, 0) * CFrame.Angles(0, 0, math.rad(20 * p569)));
						end);
						u611 = l__Main__1967.CFrame;
						Utilities.Tween(0, nil, function(p570)
							Utilities.MoveModel(l__Main__1967, u611 * CFrame.new(2.625 * p570, 7.875 * p570, 0));
						end);
					end);
					Utilities.Tween(0, nil, function(p571)
						Utilities.MoveModel(l__Main__1963, u610 * CFrame.new(0, 10.5 * p571, 0));
					end);
				end);
				local l__CFrame__612 = l__Main__1965.CFrame;
				Utilities.Tween(0, nil, function(p572)
					Utilities.MoveModel(l__Main__1965, l__CFrame__612 * CFrame.new(-54 * p572, 0, 0));
				end);
			end
		end,
		onLoad_chunk53 = function(chunk)
			local v1924 = nil;
			local v1925 = nil;
			local l__Tess__1926 = chunk.npcs.Tess;
			v1925 = chunk.map.Boat;
			v1924 = chunk.npcs.TheFisherman;
			local v1927 = l__Tess__1926.humanoid:LoadAnimation(create("Animation")({
				AnimationId = "rbxassetid://" .. _p.animationId.NPCWave
			}));
			if not completedEvents.tessdecca then
				local l__CurrentCamera__592 = workspace.CurrentCamera;
				local l__Jake__593 = chunk.npcs.Jake;
				touchEvent("tessdecca", chunk.map.CutsceneTrigger, true, function()
					spawn(function()
						_p.Menu:disable();
					end);
					MasterControl.WalkEnabled = false;
					MasterControl:Stop();
					_p.RunningShoes:disable();
					local u594 = _p.player.Character.HumanoidRootPart.CFrame;
					spawn(function()
						l__Tess__1926:LookAt(u594);
					end);
					Utilities.exclaim(l__Tess__1926.model.Head);
					v1927:Play();
					wait(1);
					MasterControl:WalkTo(Vector3.new(-726.794, 27.45, 7648.458));
					u594 = _p.player.Character.HumanoidRootPart.CFrame;
					local u595 = l__Tess__1926.model.HumanoidRootPart.CFrame;
					spawn(function()
						MasterControl:LookAt(u595);
					end);
					spawn(function()
						l__Tess__1926:LookAt(u594);
					end);
					l__Tess__1926:Say("I've got good news, " .. _p.PlayerData.trainerName .. "!", "The older couple that run the fish market here said that we can try to catch a ride to Crescent Island with one of their fishermen.", "He rides all the way out to Crescent Island to gather fish for their market", "He's down at the beach about to take off right now, so we'll to go catch him quickly.", "Let's go.");
					local v1928 = { Vector3.new(-738.459, 27.306, 7663.974), Vector3.new(-740.705, 17.287, 7689.639), Vector3.new(-768.923, 5.662, 7694.413), Vector3.new(-826.527, 4.624, 7732.488) };
					local u596 = { Vector3.new(-738.459, 27.306, 7663.974), Vector3.new(-740.705, 17.287, 7689.639), Vector3.new(-768.923, 5.662, 7694.413), Vector3.new(-824.691, 4.624, 7736.136) };
					spawn(function()
						wait(0.5);
						for v1929, v1930 in pairs(u596) do
							MasterControl:WalkTo(v1930);
						end;
					end);
					for v1931, v1932 in pairs(v1928) do
						l__Tess__1926:WalkTo(v1932);
					end;
					wait(0.5);
					u595 = l__Tess__1926.model.HumanoidRootPart.CFrame;
					u594 = _p.player.Character.HumanoidRootPart.CFrame;
					local l__CFrame__1933 = v1924.model.HumanoidRootPart.CFrame;
					l__Tess__1926:LookAt(l__CFrame__1933);
					l__Tess__1926:Say("Excuse me, sir!");
					v1924:LookAt(CFrame.new(-826.527, 5.624, 7732.488));
					v1924:Say("Aye, if you want some of Kade Krabby's Klassic Karefully-Krafted Kantonian Kwik-Krisped Karp, I just gave all I caught to my boss at the market.");
					Utilities.question(l__Tess__1926.model.Head);
					l__Tess__1926:Say("Oh no, we aren't looking for seafood.", "I was hoping to ask you for help with something else.");
					v1924:Say("Oh, well alright, what is it?");
					l__Tess__1926:Say("Well, my name is Tess, and this is my friend " .. _p.PlayerData.trainerName .. ", and we need to get to Crescent Island.", "I talked with the older couple at the market, and they told me that you make fishing trips out to Crescent Island frequently.", "So, I was wondering...");
					v1924:Say("Stop it right there, missy. Do you know what kinda people are out on that island?", "They're some of the roughest, tougest, nastiest, scoundrels you'll ever meet...");
					wait(1);
					v1924:LookAt(u595);
					wait(0.5);
					v1924:LookAt(u594);
					Utilities.exclaim(v1924.model.Head);
					v1924:Say("You kids aren't serious about wanting to go there, are you?");
					l__Tess__1926:Say("Yes, we are on a mission to save our friends and family.", "We're chasing a group of goons that go by the name \"Team Eclipse.\"", "They are plotting something disastrous, and if we don't stop them, Roria could be in danger.");
					v1924:Say("Wow that is quite a tale.", "I've heard a lot of stories in my time, and yours sounds just as fishy as a fishing story.", "Still, you both look rather determined to get there, regardless of your reasons...");
					l__Tess__1926:Say("I wish we were making this up...", "We'll do whatever it takes to get to the island.");
					v1924:Say("Is that so?", "Well, I supposed our only obstacle now is that I can really only take one extra person on my boat.");
					u595 = l__Tess__1926.model.HumanoidRootPart.CFrame;
					u594 = _p.player.Character.HumanoidRootPart.CFrame;
					l__Tess__1926:LookAt(u594);
					spawn(function()
					...
					l__Tess__1926:Say("Okay, I have a new plan.", "Remember how Brad gave you the Hidden Machine for teaching Surf?", "I will ride in the boat with this guy to the island, and you Surf your way there.", "I know you, and I know you can handle the waters from here to there.");
					l__Tess__1926:Say("[small]And don't worry about me getting on a boat with a stranger. Garchomp can take this guy, easily. I mean, just look at him. His best move is probably Struggle...");
					l__Tess__1926:LookAt(l__CFrame__1933);
					l__Tess__1926:Say("Alright, it's settled.", "I'll ride in the boat with you, and " .. _p.PlayerData.trainerName .. " will Surf to Crescent Island.");
					v1924:Say("Aight, if I can't talk you out of going, I would at least recommend stocking up on items and healing your Pokemon before we depart.");
					l__Tess__1926:LookAt(u594);
					l__Tess__1926:Say("I already just healed my team!", "Make sure you do the same.", "Come find me when you get to Crescent Island!");
					l__Tess__1926:LookAt(l__CFrame__1933);
					l__Tess__1926:Say("Ok, I'm ready to go!");
					v1924:Say("Alright, hop it and we'll be off.");
					l__CurrentCamera__592.CameraType = Enum.CameraType.Scriptable;
					spawn(function()
					...
					spawn(function()
						v1924:LookAt(v1925.FishermanPos.CFrame);
					end);
					l__Tess__1926:LookAt(v1925.TessPos.CFrame);
					local l__HumanoidRootPart__1934 = v1924.model.HumanoidRootPart;
					local l__CFrame__597 = l__HumanoidRootPart__1934.CFrame;
					spawn(function()
						Utilities.Tween(1, nil, function(p550)
							Utilities.MoveModel(l__HumanoidRootPart__1934, l__CFrame__597 * CFrame.new(0, 5 * math.sin(math.pi * p550), -8 * p550));
						end);
						l__HumanoidRootPart__1934.Anchored = true;
						l__HumanoidRootPart__1934.CFrame = v1925.FishermanPos.CFrame;
					end);
					wait(0.2);
					local l__HumanoidRootPart__1935 = l__Tess__1926.model.HumanoidRootPart;
					local l__CFrame__598 = l__HumanoidRootPart__1935.CFrame;
					Utilities.Tween(1, nil, function(p551)
						Utilities.MoveModel(l__HumanoidRootPart__1935, l__CFrame__598 * CFrame.new(0, 5 * math.sin(math.pi * p551), -10 * p551));
					end);
					l__HumanoidRootPart__1935.Anchored = true;
					l__HumanoidRootPart__1935.CFrame = v1925.TessPos.CFrame;
					local l__RunService__1936 = game:GetService("RunService");
					l__RunService__1936:BindToRenderStep("DeccaBoat", Enum.RenderPriority.Camera.Value - 1, function()
						l__HumanoidRootPart__1935.CFrame = v1925.TessPos.CFrame;
						l__HumanoidRootPart__1934.CFrame = v1925.FishermanPos.CFrame;
					end);
					spawn(function()
						local l__Main__1937 = v1925.Main;
						local l__CFrame__599 = l__Main__1937.CFrame;
						Utilities.Tween(10, "easeInSine", function(p552)
							Utilities.MoveModel(l__Main__1937, l__CFrame__599 * CFrame.Angles(0, math.rad(90 * p552), 0) * CFrame.new(700 * p552, 0, 0));
						end);
						l__RunService__1936:UnbindFromRenderStep("DeccaBoat");
						l__Tess__1926:Remove();
						v1924:Remove();
						v1925:Remove();
					end);
					l__Jake__593:Teleport(CFrame.new(-731.514, 27.443, 7765.493));
					l__Jake__593:LookAt(CFrame.new(-830.849, 5.377, 7739.622));
					local l__CFrame__600 = l__CurrentCamera__592.CFrame;
					...
					Utilities.Tween(8.5, "linear", function(p553)
						l__CurrentCamera__592.CFrame = l__CFrame__600:Lerp(u601, p553);
					end);
					wait(3);
					l__Jake__593:WalkTo(Vector3.new(-731.515, 27.442, 7798.938));
					l__Jake__593:Remove();
					Utilities.lookBackAtMe();
					MasterControl.WalkEnabled = true;
					_p.RunningShoes:enable();
					...
					spawn(function()
						_p.PlayerData:completeEvent("tessdecca");
					end);
				end);
				return;
			end;
			l__Tess__1926:Teleport(CFrame.new(-622.68, 8.026, 7734.233));
			v1924:Teleport(CFrame.new(-622.68, 8.026, 7729.109));
			v1925:Remove();
		end,
		onLoad_chunk77 = function(chunk)
			touchEvent('getLugia', chunk.map.Trigger, false, function()
				if not completedEvents.getsilverwing then return false end
				MasterControl.WalkEnabled = false
				MasterControl:Stop()
				local cam = workspace.CurrentCamera
				cam.CameraType = Enum.CameraType.Scriptable
				cam.CFrame = CFrame.new(-232.304672, 1625.33948, -82.2734909, -0.258817136, 0, 0.96591866, 0, 0.99999994, 0, -0.96591866, 0, -0.258817136)
				wait(2)
				chat:say('The Silver Wing is glowing.')
				_p.Battle._SpriteClass:playCry(nil, {id = 9987803215, startTime = 3.91, duration = 1.83})
				wait(1)
				_p.Battle:doWildBattle(chunk.regionData.Lugia, {musicId = 0})
				_p.PlayerData:completeEvent('getLugia')
				MasterControl.WalkEnabled = true
				MasterControl:Stop()
			end)
		end,
		onLoad_chunk48 = function(chunk)
			...
			local dopuzzle = function(buttonname)
				...
				if stage == 8 then
					spawn(function() 
						...
						Utilities.lookBackAtMe()
						spawn(function() _p.Menu:enable() end)
						pcall(function() _p.PlayerData:completeEvent('LightDoor') end)
						MasterControl.WalkEnabled = true
					end)
				end
			end
			...
			-- Come Back Stuff
			if completedEvents.OpenGirafarigDunsparceDoor then
			...
			if completedEvents.DoWallSmashDoor then
			...
			if completedEvents.DidRegiPuzzles then
				...
				local puz = game.Players.LocalPlayer.PlayerScripts:FindFirstChild('Client.Receiver')
				local puz2 = game.Players.LocalPlayer.PlayerScripts:FindFirstChild('Client.Handler')
				
				puz2.Puzzle.Value = chunk.map.MainPuzzle
				puz.Enabled = true
				puz2.Enabled = true
			end
			if completedEvents.LightDoor then
				spawn(function()
					dolightup('Blue', true)
					dolightup('Red', true)
					dolightup('Yellow', true)
		...
		onUnload_chunk48 = function(chunk)
			local puz = game.Players.LocalPlayer.PlayerScripts:FindFirstChild('Client.Receiver')
			local puz2 = game.Players.LocalPlayer.PlayerScripts:FindFirstChild('Client.Handler')
		...
		onLoad_chunk49 = function(chunk)
			if completedEvents.getRegSteel then
				chunk.map.Registeel:Remove()
			end
		end,
		onLoad_chunk50 = function(chunk)
			if completedEvents.getRegRock then
				chunk.map.Regirock:Remove()
			end
		end,
		onLoad_chunk58 = function(chunk)
			if completedEvents.getRegIce then
				chunk.map.Regice:Remove()
			end
		end,
		onLoad_chunk66 = function(chunk)
			local l__map__1938 = chunk.map
			local l__Cobalion__1939 = l__map__1938:FindFirstChild("Cobalion")
			local l__Terrakion__1940 = l__map__1938:FindFirstChild("Terrakion")
			local l__Virizion__1941 = l__map__1938:FindFirstChild("Virizion")
			local l__Sword__1943 = l__map__1938.Sword
			if completedEvents.SOJ then
				if _p.Connection:get('PDS', 'hasSOJ') and not completedEvents.SOJ2 then
					local Keldeo = _p.DataManager:request({'Model', 'Keldeo'})
					Keldeo.Parent = chunk.map
				end
				l__Cobalion__1939:Remove()
				l__Terrakion__1940:Remove()
				l__Virizion__1941:Remove()
				l__Sword__1943:Remove()
				return
			end
			if l__Cobalion__1939 and l__Terrakion__1940 and l__Virizion__1941 then
				touchEvent('SOJ', chunk.map.TRIGGER, true, function()
					local l__CurrentCamera__1942 = workspace.CurrentCamera
					l__CurrentCamera__1942.CameraType = Enum.CameraType.Scriptable
					...
					wait(1)
					l__Sword__1943.Size = Vector3.new(1.515, 0.36, 5.665)
					l__Sword__1943.CFrame = l__Sword__1943.CFrame + Vector3.new(0, 10, 0)
					Utilities.Tween(1, "easeOutCubic", function(p556)
						l__Sword__1943.Transparency = 1 - 0.8 * p556
					end)
					local l__Orientation__1944 = l__Sword__1943.Orientation
					local u602 = l__Sword__1943.CFrame
					Utilities.Tween(1, nil, function(p557)
						l__Sword__1943.CFrame = u602 * CFrame.new(0, 0, 10 * p557) * CFrame.Angles(0, math.rad(-360 * p557), 0)
						l__Sword__1943.Size = Vector3.new(1.515 + 5.929 * p557, 0.36 + 1.41 * p557, 5.665 + 22.164 * p557)
						l__Sword__1943.Transparency = 0.2 - 0.2 * p557
					end)
					u602 = l__Sword__1943.CFrame
					Utilities.Tween(0.4, "easeInSine", function(p558)
						l__Sword__1943.CFrame = u602 * CFrame.new(0, 0, -10 * p558)
					end)
					...
					l__CurrentCamera__1942.CameraType = Enum.CameraType.Custom
		...
		onLoad_chunk42 = function(chunk)
			if completedEvents.OpenRDoor then
				...
				local DoorCFrame = DoorMain.CFrame
				spawn(function()
					pcall(function() rDoor['#InanimateInteract']:Remove() end)
					Utilities.Tween(.1, "easeInSine", function(p87)
						Utilities.MoveModel(DoorMain, DoorCFrame + Vector3.new(0, 10 * p87, 0))
					end)
				end)
		...
		onLoad_chunk59 = function(chunk)
			if completedEvents.findRegigigas then
				pcall(function()
					chunk.map.Deadgigas:Remove()
				end)
				return
			end
			touchEvent('findRegigigas', chunk.map.Trigger, true, function()
				spawn(function() _p.Menu:disable() end)
				MasterControl.WalkEnabled = false
				MasterControl:Stop()
				workspace.CurrentCamera.CameraType = Enum.CameraType.Scriptable
		...
		onLoad_chunk69 = function(p451)
			local l__Lighting__533 = game.Lighting;
			local u534 = {
				pos1 = CFrame.new(25.383, 0.831, 5187.016), 
				pos2 = CFrame.new(5.021, 0.831, 5127.564), 
				pos3 = CFrame.new(-77.995, 0.831, 5115.42), 
				pos4 = CFrame.new(-444.363, 0.831, 4815.131), 
				pos5 = CFrame.new(-448.539, 0.831, 4730.805), 
				pos6 = CFrame.new(-550.234, 0.831, 4462.768), 
				pos7 = CFrame.new(-605.84, 0.831, 4455.687), 
				pos8 = CFrame.new(-569.302, 0.831, 4437.501), 
				pos9 = CFrame.new(-185.327, 0.831, 4659.189), 
				pos10 = CFrame.new(-168.164, 0.831, 4714.135), 
				pos11 = CFrame.new(-156.204, 0.831, 4670.055), 
				pos12 = CFrame.new(482.798, 0.831, 5367.738), 
				pos13 = CFrame.new(441.157, 0.831, 5402.645), 
				pos14 = CFrame.new(502.376, 0.831, 5405.072)
			};
			local function u535()
				if _p.DataManager.currentChunk.id ~= "chunk69" then
					return;
				end;
				local v1660 = l__Lighting__533:GetMinutesAfterMidnight() / 60;
				if not (v1660 < 6) and not (v1660 >= 18) then
					if v1660 > 6 or v1660 < 18 then
						u532 = false;
						if p451.map:FindFirstChild("SandSpot") then
							p451.map.SandSpot:Remove();
						end;
					end;
					return;
				end;
				...
				u532 = true;
				local v1661 = game.ReplicatedStorage.Models.Misc.SandSpot:Clone();
				v1661.Parent = p451.map;
				v1661.CFrame = u534["pos" .. math.random(14)];
				local u536 = false;
				v1661.Touched:Connect(function()
					if u536 then
						return;
					end;
					u536 = true;
					...
					_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Sand, {
						battleSceneType = "Route17Land"
					});
					_p.MasterControl.WalkEnabled = true;
					u536 = false;
				end);
			end;
			table.insert({}, l__Lighting__533.Changed:connect(function(p452)
				if p452 ~= "TimeOfDay" then
					return;
				end;
				u535();
			end));
		...
		onLoad_chunk54 = function(chunk)
			...
			if completedEvents.DefeatHoopa then
				chunk.npcs.BARRELGUY:Remove()
				chunk.map.Gymstop:Remove()
			end
			...
			if not completedEvents.dofisherman then
				touchEvent('dofisherman', chunk.map.cTrigger1, true, function()
					spawn(function() _p.Menu:disable() end)
					MasterControl.WalkEnabled = false
					MasterControl:Stop()
			...
			if not completedEvents.doeliman then
				touchEvent('doeliman', chunk.map.cTrigger2, true, function()
					MasterControl.WalkEnabled = false
					MasterControl:Stop()
					_p.Menu:disable()
					workspace.CurrentCamera.CameraType = Enum.CameraType.Scriptable
					workspace.CurrentCamera.CFrame = CFrame.new(-1722.83984, 878.017761, -409.545807, -0.965925753, 0, 0.258819103, 0, 1, 0, -0.258819103, 0, -0.965925753)
					cutgrunt:LookAt(_p.player.Character.Head.Position)
					Utilities.exclaim(cutgrunt.model.Head)
					local tdoor = chunk:getDoor('Tavern')
					spawn(function() cutgrunt:WalkTo(Vector3.new(-1722.075, 872.338, -374.826)) end)
					wait(.5)
					tdoor:open(.3)
					wait(.2)
					tdoor:close(.3)
		...
		onLoad_chunk57 = function(chunk)
			touchEvent(nil, chunk.map.ElevatorTrigger, false, function()
				local l__CurrentCamera__645 = workspace.CurrentCamera;
				local u646 = CFrame.new(-91.2821198, 334.370117, -753.983215, -0.999974251, 0.002645534, -0.00667580543, 2.32830644E-10, 0.929662287, 0.368412912, 0.00718089286, 0.368403435, -0.929638326);
				local l__Elevator__2034 = chunk.map.Elevator;
				l__CurrentCamera__645.CameraType = Enum.CameraType.Scriptable;
				spawn(function()
				...
				MasterControl.WalkEnabled = false;
				(function(p608)
					Tween(1.2, nil, function(p610)
						l__CurrentCamera__645.CFrame = u646 * CFrame.new(0, math.cos(math.random() * math.pi * 2) * ((1 - p610) * p608), 0);
					end);
				end)(0.2);
				local u647 = l__Elevator__2034;
				local l__CFrame__648 = l__Elevator__2034.Main.CFrame;
				Tween(2, "easeInSine", function(p611)
					Utilities.MoveModel(u647.Main, l__CFrame__648 + Vector3.new(0, 10 * p611, 0));
				end);
				_p.MusicManager:popMusic("all", 1);
				Utilities.FadeOut(1);
				Utilities.TeleportToSpawnBox();
				...
				wait(0.5);
				wait();
				local v2035 = _p.DataManager:loadChunk("chunk54");
				v2035.indoors = true;
				MasterControl:SetIndoors(true);
				local v2036 = v2035:getRoom("Tavern", v2035:getDoor("Tavern"), 1);
				v2035.roomStack = { v2036 };
				local u649 = CFrame.new(1194.23425, 19.6975975, 1017.59741, -0.999953687, 0.00624616956, -0.00732445344, -4.65661287E-10, 0.760893404, 0.648876965, 0.00962612405, 0.648846924, -0.760858119);
				v2035:bindIndoorCam();
				u647 = v2036.model.Elevator;
				Utilities.Teleport(v2036.model.TeleportSpawn.CFrame + Vector3.new(0, 0, 0));
				spawn(function()
					MasterControl:LookAt(v2036.model.TeleportSpawn.Position + Vector3.new(0, 0, -5));
				end);
				spawn(function()
					local l__CFrame__650 = workspace.CurrentCamera.CFrame;
					(function(p612)
						Utilities.Tween(1.2, nil, function(p614)
							local v2037 = (1 - p614) * p612;
							local v2038 = math.random() * math.pi * 2;
							workspace.CurrentCamera.CFrame = l__CFrame__650 * CFrame.new(math.cos(v2038) * v2037, 0, math.sin(v2038) * v2037);
						end);
					end)(0.07);
				end);
				wait();
				local l__CFrame__2039 = v2036.model.Elevator.ElevatorMain.CFrame;
				u646 = CFrame.new(1194.06641, 17.4220715, 1012.09058, -0.999999523, -0.000600461033, 0.000854510698, -5.82076609E-11, 0.818194509, 0.574941695, -0.00104438583, 0.574941397, -0.818194032);
				spawn(function()
					local l__ElevatorMain__2040 = u647.ElevatorMain;
					local l__CFrame__651 = l__ElevatorMain__2040.CFrame;
					Utilities.Tween(2, nil, function(p615)
						Utilities.MoveModel(l__ElevatorMain__2040, l__CFrame__651 + Vector3.new(0, 7.5 * p615, 0));
					end);
					spawn(function()
						local l__CFrame__652 = workspace.CurrentCamera.CFrame;
						(function(p616)
							Tween(1.2, nil, function(p618)
								l__CurrentCamera__645.CFrame = l__CFrame__652 * CFrame.new(0, math.cos(math.random() * math.pi * 2) * ((1 - p618) * p616), 0);
							end);
						end)(0.07);
					end);
					wait(1);
					v2035.roomCamDisabled = false;
					MasterControl:WalkTo(v2036.model.TeleportSpawn.Position + Vector3.new(0, 0, -6));
					_p.Menu:enable();
					_p.Events.onBeforeEnter_Tavern(v2036);
					_p.MasterControl.WalkEnabled = true;
				end);
				v2035.roomCamDisabled = true;
				l__CurrentCamera__645.CameraType = Enum.CameraType.Scriptable;
				Utilities.Tween(0.1, nil, function(p619)
					l__CurrentCamera__645.CFrame = l__CurrentCamera__645.CFrame * CFrame.new(Vector3.new(0, 7.5 * p619, 0));
				end);
				Utilities.FadeIn(1);
			end);
			if completedEvents.doebasedoor2 then
				local l__map__2031 = chunk.map;
				local l__CFrame__637 = l__map__2031.Door2.Left.Main.CFrame;
				delay(0, function()
					Utilities.Tween(0.1, "easeOutSine", function(p600)
						Utilities.MoveModel(l__map__2031.Door2.Left.Main, l__CFrame__637 + Vector3.new(-10 * p600, 0, 0));
					end);
				end);
				local l__CFrame__638 = l__map__2031.Door2.Right.Main.CFrame;
				delay(0, function()
					Utilities.Tween(0.1, "easeOutSine", function(p601)
						Utilities.MoveModel(l__map__2031.Door2.Right.Main, l__CFrame__638 + Vector3.new(10 * p601, 0, 0));
					end);
				end);
				local l__CFrame__639 = l__map__2031.Door3.Left.Main.CFrame;
				delay(0, function()
					Utilities.Tween(0.1, "easeOutCubic", function(p602)
						Utilities.MoveModel(l__map__2031.Door3.Left.Main, l__CFrame__639 + Vector3.new(-10 * p602, 0, 0));
					end);
				end);
				local l__CFrame__640 = l__map__2031.Door3.Right.Main.CFrame;
				delay(0, function()
					Utilities.Tween(0.1, "easeOutCubic", function(p603)
						Utilities.MoveModel(l__map__2031.Door3.Right.Main, l__CFrame__640 + Vector3.new(10 * p603, 0, 0));
					end);
				end);
				local l__CFrame__641 = l__map__2031.Door4.Left.Main.CFrame;
				delay(0, function()
					Utilities.Tween(0.1, "easeOutCubic", function(p604)
						Utilities.MoveModel(l__map__2031.Door4.Left.Main, l__CFrame__641 + Vector3.new(0, 0, 7 * p604));
					end);
				end);
				local l__CFrame__642 = l__map__2031.Door4.Right.Main.CFrame;
				delay(0, function()
					Utilities.Tween(0.1, "easeOutCubic", function(p605)
						Utilities.MoveModel(l__map__2031.Door4.Right.Main, l__CFrame__642 + Vector3.new(0, 0, -7 * p605));
					end);
				end);
				local l__CFrame__643 = l__map__2031.Door5.Left.Main.CFrame;
				delay(0, function()
					Utilities.Tween(0.1, "easeOutCubic", function(p606)
						Utilities.MoveModel(l__map__2031.Door5.Left.Main, l__CFrame__643 + Vector3.new(0, 0, -7 * p606));
					end);
				end);
				local l__CFrame__644 = l__map__2031.Door5.Right.Main.CFrame;
				delay(0, function()
					Utilities.Tween(0.1, "easeOutCubic", function(p607)
						Utilities.MoveModel(l__map__2031.Door5.Right.Main, l__CFrame__644 + Vector3.new(0, 0, 7 * p607));
					end);
				end);
			end
			spawn(function()
		...
		onLoad_chunk571 = function(chunk)
			spawn(function()
				wait(.1)
				if completedEvents.EBDefeat then
					for i,obj in pairs(chunk.map:GetChildren()) do
		...
		onLoad_chunk572 = function(chunk)
		...
		onLoad_chunk576 = function(chunk)
			if completedEvents.getgen then
				chunk.map.Gen:Remove()
			end
		...
		onLoad_chunk573 = function(chunk)
			spawn(function()
				wait(.1)
				if completedEvents.EBDefeat then
					for i,obj in pairs(chunk.map:GetChildren()) do
		...
		onLoad_chunk574 = function(chunk)
			if completedEvents.doebasedoor2 then
			...
			if not completedEvents.doebasedoor then
				touchEvent('doebasedoor', chunk.map.TIGGER, false, function()
					MasterControl.WalkEnabled = false
					local cam = workspace.CurrentCamera
					cam.CameraType = Enum.CameraType.Scriptable
		...
		onLoad_chunk575 = function(chunk)
			if completedEvents.doebasecard then
				chunk.map['Card Key']:Remove()
			end
		end,

		onLoad_chunk56 = function(p723)
			local l__Utilities__1 = _p.Utilities;
			local p1 = _p
			local l__MasterControl__2 = _p.MasterControl
			local l__Tween__16 = Utilities.Tween
			local l__Players__4 = game:GetService("Players");
			local u7 = p1.PlayerData.completedEvents;
			local u8 = p1.NPCChat;
			local u9 = u8.interactableNPCs;
			local u10 = p1.Battle._SpriteClass;
			local function u82()
				l__Utilities__1.sound(304774035, nil, nil, 10);
			end;
			local l__Heartbeat__6 = game:GetService("RunService").Heartbeat;
			local l__Create__2 = l__Utilities__1.Create;
			p1.DataManager:preload(10013499719, 10013500682, 9995399504, 10025812298, 10025811365, 2648568095);
			local l__trainerName__2324 = p1.PlayerData.trainerName;
			local l__Position__2325 = p1.player.Character.HumanoidRootPart.Position;
			local l__LHC_Admin__2326 = p723.npcs.LHC_Admin;
			local l__Jake__2327 = p723.npcs.Jake;
			local v2328 = Vector3.new(-180.803, 1792.624, 8.263);
			local l__Tess__2329 = p723.npcs.Tess;
			local l__EclipseAdmin__2330 = p723.npcs.EclipseAdmin;
			local l__Professor__2331 = p723.npcs.Professor;
			local l__Looker__2332 = p723.npcs.Looker;
			local l__Mom__2333 = p723.npcs.Mom;
			local l__Tyler__2334 = p723.npcs.Tyler;
			local l__Lighting__2335 = game:GetService("Lighting");
			local l__Dad__2336 = p723.npcs.Dad;
			local l__Chains__2337 = p723.map:FindFirstChild("Chains");
			local l__1Main1__2338 = p723.map.Chains:FindFirstChild("1Main1");
			local l__1Main2__2339 = p723.map.Chains:FindFirstChild("1Main2");
			local l__1Main3__2340 = p723.map.Chains:FindFirstChild("1Main3");
			local l__1Main4__2341 = p723.map.Chains:FindFirstChild("1Main4");
			local l__2Main1__2342 = p723.map.Chains:FindFirstChild("2Main1");
			local l__2Main2__2343 = p723.map.Chains:FindFirstChild("2Main2");
			local l__2Main3__2344 = p723.map.Chains:FindFirstChild("2Main3");
			local l__2Main4__2345 = p723.map.Chains:FindFirstChild("2Main4");
			local l__Bricks__2346 = p723.map.Bricks;
			local l__Rocks__2347 = p723.map:FindFirstChild("Rocks");
			local function v2348(p724)
				local v2349 = p724.humanoid:GetPlayingAnimationTracks()[1];
				v2349:Stop();
			...
			v2348(l__Tess__2329);
			v2348(l__LHC_Admin__2326);
			v2348(l__Mom__2333);
			v2348(l__EclipseAdmin__2330);
			v2348(l__Professor__2331);
			v2348(l__Dad__2336);
			v2348(l__Jake__2327);
			if u7.DefeatHoopa then
				for v2350, v2351 in pairs(p723.npcs) do
					if v2351.model:FindFirstChild("IsTeamEclipse") then
						v2351:Remove();
					end;
				end;
			end;
			if not u7.DefeatHoopa then
				p723.map.BrokenChains:Remove();
			end;
			if not u7.DefeatHoopa then
				local l__CurrentCamera__772 = workspace.CurrentCamera;
				local u773 = p723;
				touchEvent(nil, p723.map.HTrigger, false, function()
					l__MasterControl__2.WalkEnabled = false;
					local u774 = CFrame.new(-290.399933, 1793.9845, -31.5771103, -0.527084649, 0.0803968981, -0.846001327, -0, 0.995514989, 0.0946054161, 0.849812865, 0.0498650633, -0.52472055);
					local u775 = CFrame.new(-324.297424, 1795.59534, 6.97160101, -0.00261850376, 0.012573122, -0.999917507, 3.63797924E-12, 0.999921024, 0.012573164, 0.999996662, 3.29228751E-05, -0.00261829654);
					p1.MasterControl:Stop();
					u773.map.HoopaBarrier.Parent = l__Lighting__2335;
					l__MasterControl__2:Stop();
					l__CurrentCamera__772.CameraType = Enum.CameraType.Scriptable;
					spawn(function()
						p1.MusicManager:prepareToStack(1);
					end);
					l__Utilities__1.FadeOut(1);
					u8:disable();
					p1.RunningShoes:disable();
					...
					l__CurrentCamera__772.CFrame = v2352;
					wait(1);
					spawn(function()
						l__Utilities__1.FadeIn(1);
						p1.player.Character.HumanoidRootPart.Anchored = false;
					end);
					p1.MusicManager:stackMusic(10013499719, "Cutscene");
					wait(1.8);
					spawn(function()
						l__Utilities__1.Tween(1.8, "easeInOutQuad", function(p725)
							l__CurrentCamera__772.CFrame = v2352:Lerp(v2353, p725);
						end);
						l__Professor__2331:LookAt(Vector3.new(-301.732, 1765.59, 6.84));
					end);
					l__MasterControl__2:WalkTo(Vector3.new(-306.504, 1765.637, 7.302));
					spawn(function()

					end);
					l__Utilities__1.Tween(1, "easeInOutQuad", function(p726)
						l__CurrentCamera__772.CFrame = v2353:Lerp(v2354, p726);
					end);
					l__MasterControl__2:LookAt(v2328);
					spawn(function()
						l__LHC_Admin__2326:LookAt(Vector3.new(-306.504, 1765.637, 7.302));
					end);
					spawn(function()
						l__Jake__2327:LookAt(Vector3.new(-306.504, 1765.637, 7.302));
					end);
					spawn(function()
						l__Tyler__2334:LookAt(Vector3.new(-306.504, 1765.637, 7.302));
					end);
					spawn(function()
						l__Tess__2329:LookAt(Vector3.new(-306.504, 1765.637, 7.302));
					end);
					spawn(function()
						l__Mom__2333:LookAt(Vector3.new(-306.504, 1765.637, 7.302));
					end);
					spawn(function()
						l__Dad__2336:LookAt(Vector3.new(-306.504, 1765.637, 7.302));
					end);
					spawn(function()
						l__EclipseAdmin__2330:LookAt(Vector3.new(-306.504, 1765.637, 7.302));
					end);
					l__Professor__2331:Say("Hahahahahaha... You never give up, do you?");
					l__Professor__2331:Say("You just don't know when to quit.", "I knew that about you, ever since that little incident with Linda in Gale Forest.", "Your parents thought they could hide that precious key by giving it to you.");
					spawn(function()
						l__Professor__2331:LookAt(Vector3.new(-291.639, 1768.13, -15.132));
					end);
					spawn(function()
						l__Dad__2336:LookAt(Vector3.new(-290.286, 1768.384, 7.302));
					end);
					spawn(function()
						l__Mom__2333:LookAt(Vector3.new(-290.286, 1768.384, 7.302));
					end);
					spawn(function()
						l__Tess__2329:LookAt(Vector3.new(-290.286, 1768.384, 7.302));
					end);
					spawn(function()
						l__Jake__2327:LookAt(Vector3.new(-290.286, 1768.384, 7.302));
					end);
					l__Dad__2336:Say("We were right to do so.", "We knew after we had shown you the key, your fascination with it was too serious.", "By sending it with " .. p1.PlayerData.trainerName .. ", it would be far away from your hands.");
					l__Professor__2331:Say("And so it was, for quite some time.");
					l__Professor__2331:LookAt(Vector3.new(-306.504, 1765.637, 7.302));
					l__Professor__2331:Say("You see, there was no easy way to steal it from you.", "As it turns out, it was easier to have you deliver it personally.", "And here we are, all gathered together, exactly as I planned.");
					l__CurrentCamera__772.CFrame = CFrame.new(-307.935181, 1781.22742, -18.7227917, -0.861313164, 0.178682476, -0.475617766, 1.49011612E-08, 0.936118245, 0.351685524, 0.508074522, 0.302911371, -0.806290925);
					spawn(function()
						l__Tyler__2334:LookAt(Vector3.new(-290.273, 1768.1, 7.302));
					end);
					l__Professor__2331:LookAt(Vector3.new(-299.395, 1767.959, 29.259));
					l__Professor__2331:Say("Hit it, Tyler!");
					l__Tyler__2334:LookAt(Vector3.new(-299.473, 1768.884, 27.001));
					wait(1);
					spawn(function()
						spawn(function()
							l__Professor__2331:LookAt(Vector3.new(-269.403, 1772.124, 8.067));
						end);
						spawn(function()
							l__LHC_Admin__2326:LookAt(Vector3.new(-269.403, 1772.124, 8.067));
						end);
						spawn(function()
							l__Jake__2327:LookAt(Vector3.new(-269.403, 1772.124, 8.067));
						end);
						spawn(function()
							l__Tess__2329:LookAt(Vector3.new(-269.403, 1772.124, 8.067));
						end);
						spawn(function()
							l__Mom__2333:LookAt(Vector3.new(-269.403, 1772.124, 8.067));
						end);
						spawn(function()
							l__Dad__2336:LookAt(Vector3.new(-269.403, 1772.124, 8.067));
						end);
						l__EclipseAdmin__2330:LookAt(Vector3.new(-269.403, 1772.124, 8.067));
					end);
					l__Utilities__1.lookAt(CFrame.new(-290.679382, 1781.43359, 6.22089005, -0.035363704, 0.38830629, -0.920851588, -0, 0.921427965, 0.388549328, 0.999374509, 0.0137405433, -0.0325851068));
					l__Utilities__1.sound(10025812298 , 1.5);
					local l__TheBronzeBrick__2355 = u773.map.TheBronzeBrick;
					local u776 = nil;
					spawn(function()
						u776 = p1.DataManager:request({ "Model", "Hoopa" });
					end);
					l__TheBronzeBrick__2355.Main.CFrame = CFrame.new(-307.216705, 1772.99976, 6.3908577, 0.0565521345, 0.998340368, 0.0112023987, 0.00418010913, 0.0109835258, -0.999934971, -0.998393953, 0.0565951839, -0.00355197652);
					local l__Main__2356 = l__TheBronzeBrick__2355.Main;
					local v2357 = {
						[l__Main__2356] = CFrame.new()
					};
					local u777 = false;
					local u778 = l__Main__2356.CFrame * CFrame.Angles(0, 0, 0);
					spawn(function()
						u777 = true;
						local v2358 = tick();
						while u777 do
							local v2359 = u778 + Vector3.new(0, math.sin((tick() - v2358) * 1.7) * 0.4, 0);
							for v2360, v2361 in pairs(v2357) do
								v2360.CFrame = v2359 * v2361;
							end;
							l__Heartbeat__6:wait();					
						end;
					end);
					spawn(function()
						l__Utilities__1.Tween(1, nil, function(p727)
							u773.map.BlackWire.Color = Color3.fromRGB(0 + 255 * p727, 0 + 255 * p727, 0 + 255 * p727);
						end);
					end);
					spawn(function()
						l__Utilities__1.Tween(1, nil, function(p728)
							u773.map.RedWire.Color = Color3.fromRGB(123 + 132 * p728 * p728, 46 + 209 * p728, 47 + 208 * p728);
						end);
					end);
					wait(2);
					l__Utilities__1.FadeOut(0.1, Color3.new(1, 1, 1));
					l__Utilities__1.FadeIn(0.7);
					spawn(function()
						l__Utilities__1.Tween(1, nil, function(p729)
							u773.map.BlackWire.Color = Color3.fromRGB(255 - 255 * p729, 255 - 255 * p729, 255 - 255 * p729);
						end);
					end);
					spawn(function()
						l__Tyler__2334:LookAt(Vector3.new(-269.2, 1767.123, 7.501));
					end);
					spawn(function()
						l__Utilities__1.Tween(1, nil, function(p730)
							u773.map.RedWire.Color = Color3.fromRGB(255 - 132 * p730, 255 - 209 * p730, 255 - 208 * p730);
						end);
					end);
					wait(1);
					l__Utilities__1.lookAt(CFrame.new(-316.580322, 1775.29419, -1.69024003, -0.306802899, 0.160335407, -0.93817091, -0, 0.985708714, 0.168459684, 0.951773167, 0.0516839176, -0.302418232));
					l__Professor__2331:Say("We've awakened the portal...");
					l__Professor__2331:LookAt(Vector3.new(-306.504, 1765.637, 7.302));
					spawn(function()
						l__Dad__2336:LookAt(Vector3.new(-306.504, 1765.637, 7.302));
					end);
					...
					spawn(function()
						l__Utilities__1.exclaim(l__Dad__2336.model.Head);
					end);
					spawn(function()
						l__Utilities__1.exclaim(l__Mom__2333.model.Head);
					end);
					l__Professor__2331:Say("...and she summons her missing piece.");
					local l__CurrentCamera__2362 = workspace.CurrentCamera;
					l__CurrentCamera__772.CFrame = CFrame.new(-259.170563, 1785.19775, 7.87359333, -0.0288365781, -0.552259147, 0.833173692, -0, 0.833520293, 0.552488923, -0.999584138, 0.0159318894, -0.0240358729);
					spawn(function()
						l__Professor__2331:LookAt(v2328);
					end);
					spawn(function()
						l__Dad__2336:LookAt(v2328);
					end);
					spawn(function()
						l__Mom__2333:LookAt(v2328);
					end);
					spawn(function()
						l__Jake__2327:LookAt(v2328);
					end);
					spawn(function()
						l__Tess__2329:LookAt(v2328);
					end);
					u777 = false;
					l__TheBronzeBrick__2355.Main.Attachment.ShineParticles:Remove();
					l__Utilities__1.Tween(1, "easeInOutSine", function(p731)
						l__TheBronzeBrick__2355.Main.CFrame = l__TheBronzeBrick__2355.Main.CFrame:Lerp(CFrame.new(-269.581818, 1778.58882, 7.95, 0.999985456, -0.00426854985, 0.00413344102, 0.00422275206, 0.999934077, 0.0110011613, -0.00418015337, -0.0109835379, 0.999934971), p731);
					end);
					for v2363, v2364 in pairs(l__TheBronzeBrick__2355:GetChildren()) do
						if v2364:IsA("BasePart") or v2364:IsA("UnionOperation") then
							v2364.Anchored = true;
						end;
					end;
					for v2365, v2366 in pairs(l__TheBronzeBrick__2355.Main:GetChildren()) do
						if v2366:IsA("Weld") then
							v2366:Remove();
						end;
					end;
					l__TheBronzeBrick__2355.Main.Glow1.Glowstuff.Enabled = true;
					l__TheBronzeBrick__2355.Main.Glow2.Glowstuff.Enabled = true;
					l__TheBronzeBrick__2355.Main.Glow3.Glowstuff.Enabled = true;
					l__TheBronzeBrick__2355.Main.Glow4.Glowstuff.Enabled = true;
					l__TheBronzeBrick__2355.Main.Glow5.Glowstuff.Enabled = true;
					l__TheBronzeBrick__2355.Main.Glow6.Glowstuff.Enabled = true;
					l__TheBronzeBrick__2355.Main.Glow7.Glowstuff.Enabled = true;
					l__TheBronzeBrick__2355.Main.Glow8.Glowstuff.Enabled = true;
					local l__Main__779 = l__TheBronzeBrick__2355.Main;
					l__Tween__16(2, nil, function(p732)
						l__Utilities__1.ScaleModel(l__Main__779, (1 + 0.016 + p732) / 1);
					end);
					l__TheBronzeBrick__2355.Main.Glow1.Glowstuff.Enabled = false;
					l__TheBronzeBrick__2355.Main.Glow2.Glowstuff.Enabled = false;
					l__TheBronzeBrick__2355.Main.Glow3.Glowstuff.Enabled = false;
					l__TheBronzeBrick__2355.Main.Glow4.Glowstuff.Enabled = false;
					l__TheBronzeBrick__2355.Main.Glow5.Glowstuff.Enabled = false;
					l__TheBronzeBrick__2355.Main.Glow6.Glowstuff.Enabled = false;
					l__TheBronzeBrick__2355.Main.Glow7.Glowstuff.Enabled = false;
					l__TheBronzeBrick__2355.Main.Glow8.Glowstuff.Enabled = false;
					local v2367 = CFrame.new(-257.25235, 1780.41272, -7.69355345, -0.672260582, -0.291714817, 0.680417657, 1.4901163E-08, 0.919092536, 0.394041657, -0.740314662, 0.264898688, -0.617869675);
					local v2368 = CFrame.new(-258.614655, 1780.49573, -8.25816345, -0.999999642, 0.00035437569, -0.000732414657, -0, 0.90016818, 0.435542554, 0.000813641935, 0.435542405, -0.900167882);
					delay(0.4, function()
						p1.MusicManager:popMusic("Cutscene", 0.1, true);
					end);
					delay(0.3, function()
						l__Utilities__1.sound(10025811365);
					end);
					l__Utilities__1.Tween(0.5, "easeInBack", function(p733)
						l__Utilities__1.MoveModel(l__TheBronzeBrick__2355.Main, l__TheBronzeBrick__2355.Main.CFrame * CFrame.new(0, -0.1 * p733, 0));
					end);
					l__Utilities__1.FadeOut(0.1, Color3.new(1, 1, 1));
					l__TheBronzeBrick__2355.Necklace:Remove();
					wait(1);
					l__Utilities__1.sound(10013500682);
					l__Utilities__1.FadeIn(1);
					local v2369 = CFrame.new(-290.399933, 1793.9845, -31.5771103, -0.527084649, 0.0803968981, -0.846001327, -0, 0.995514989, 0.0946054161, 0.849812865, 0.0498650633, -0.52472055);
					local v2370 = CFrame.new(-278.965515, 1774.97278, -6.56565857, -0.117967136, -0.321346939, 0.93958497, 3.7252903E-09, 0.946191728, 0.323606521, -0.993017495, 0.0381749384, -0.111619532);
					l__CurrentCamera__2362.CFrame = v2369;
					local function u780(p734, p735)
						l__Tween__16(1.2, nil, function(p736)
							local v2371 = (1 - p736) * p734;
							local v2372 = math.random() * math.pi * 2;
							l__CurrentCamera__772.CFrame = u774 * CFrame.new(math.cos(v2372) * v2371, 0, math.sin(v2372) * v2371);
						end);
					end;
					spawn(function()
						u780(1);
					...
					spawn(function()
						for v2373, v2374 in pairs(l__Chains__2337:GetChildren()) do
							if v2374:IsA("BasePart") then
								v2374.Anchored = false;
							end;
						end;
					end);
					wait(1.5);
					spawn(function()
						wait(1);
						l__Utilities__1.sound(2648568095, 1.4);
					end);
					spawn(function()
						u780(1.2);
					end);
					wait(1);
					for v2375, v2376 in pairs(l__Rocks__2347:GetChildren()) do
						if v2376:IsA("MeshPart") then
							v2376.Anchored = false;
						end;
					end;
					wait(2);
					l__Utilities__1.Tween(1, "easeInOutQuad", function(p737)
						l__CurrentCamera__2362.CFrame = v2369:Lerp(v2370, p737);
					end);
					l__Professor__2331:LookAt(l__EclipseAdmin__2330.model.HumanoidRootPart.CFrame);
					delay(1.5, function()
						u8:manualAdvance();
					end);
					l__Professor__2331:Say("[ma]Harry, the bottle.");
					delay(1, function()
						u8:manualAdvance();
					end);
					l__EclipseAdmin__2330:Say("[ma]Yes, sir");
					l__EclipseAdmin__2330:WalkTo(CFrame.new(-293.481, 1767.953, 5.629));
					l__CurrentCamera__2362.CFrame = v2367;
					local v2377 = u773.map.PrisonBottle:Clone();
					l__Professor__2331:LookAt(v2328);
					spawn(function()
						l__EclipseAdmin__2330:WalkTo(CFrame.new(-298.568, 1768.067, 0.638));
						l__EclipseAdmin__2330:LookAt(v2328);
					end);
					v2377.Parent = u773.map;
					v2377.Bottle.CFrame = u773.map.BottlePos.CFrame;
					wait(0.3);
					spawn(function()
						l__Utilities__1.Tween(1, nil, function(p738)
							l__CurrentCamera__2362.CFrame = v2367:Lerp(v2368, p738);
						end);
					end);
					l__Utilities__1.Tween(1, nil, function(p739)
						v2377.Bottle.CFrame = u773.map.BottlePos1.CFrame:Lerp(u773.map.BottlePos2.CFrame, p739) * CFrame.Angles(15 * p739, 0, 0);
					end);
					l__Utilities__1.Tween(1, nil, function(p740)
						v2377.Bottle.CFrame = u773.map.BottlePos2.CFrame:Lerp(u773.map.BottlePos3.CFrame, p740) * CFrame.Angles(12 * p740, 0, 0);
					end);
					for v2378, v2379 in pairs(v2377.Bottle:GetChildren()) do
						if v2379:IsA("Weld") then
							v2379:Remove();
						end;
					end;
					for v2380, v2381 in pairs(v2377:GetChildren()) do
						if not (not v2381:IsA("BasePart")) or not (not v2381:IsA("UnionOperation")) or v2381:IsA("MeshPart") then
							v2381.Anchored = false;
						end;
					end;
					spawn(function()
						l__Utilities__1.Tween(0.1, nil, function(p741)
							v2377.Bottle.CFrame = v2377.Bottle.CFrame * CFrame.Angles(0.1 * p741, 0, 0);
						end);
					end);
					local v2382 = CFrame.new(-259.984772, 1779.13513, -7.24927807, -0.999699831, 0.0097135948, -0.0224914234, -0, 0.918041766, 0.396483809, 0.0244993474, 0.396364808, -0.917766213);
					local v2383 = CFrame.new(-262.342255, 1781.47009, -18.1586399, -0.806933761, -0.0282631088, -0.589965284, -0, 0.998854399, -0.0478515178, 0.590641856, -0.0386130065, -0.806009471);
					v2377.Bottle.Anchored = false;
					wait(0.5);
					u773.map.GhostBeam.Beam.Enabled = true;
					l__Utilities__1.Tween(1, "easeInOutQuad", function(p742)
						l__CurrentCamera__2362.CFrame = v2382:Lerp(v2383, p742);
					end);
					wait(1);
					u773.map.GhostBeam.Beam.Enabled = false;
					spawn(function()
						l__Tyler__2334:LookAt(v2328);
					end);
					...
					u773.map.BlindingLight.Transparency = 0;
					v2377:Remove();
					u773.map.Shadow.Parent = l__Lighting__2335;
					spawn(function()
						local l__BlindingLight__781 = u773.map.BlindingLight;
						l__Tween__16(4, nil, function(p743)
							l__BlindingLight__781.Transparency = 0 + 1 * p743;
						end);
					end);
					l__Utilities__1.Tween(3, "easeInOutSine", function(p744)
						l__CurrentCamera__772.CFrame = v2384:Lerp(v2385, p744);
					end);
					l__Utilities__1.Tween(2, "easeInOutSine", function(p745)
						l__CurrentCamera__772.CFrame = v2385:Lerp(v2386, p745);
					end);
					l__Utilities__1.Tween(3, "easeInOutSine", function(p746)
						l__CurrentCamera__772.CFrame = v2386:Lerp(v2387, p746);
					end);
					...
					l__Lighting__2335.Shadow.Parent = u773.map;
					local l__map__2390 = u773.map;
					local v2391 = {
						hoopaIdle = u776.AnimationController:LoadAnimation(l__Create__2("Animation")({
							AnimationId = "rbxassetid://" .. p1.animationId.hoopaIdle
						})), 
						hoopaAttack = u776.AnimationController:LoadAnimation(l__Create__2("Animation")({
							AnimationId = "rbxassetid://" .. p1.animationId.hoopaAttack
						})), 
						hoopaIdle2 = u776.AnimationController:LoadAnimation(l__Create__2("Animation")({
							AnimationId = "rbxassetid://" .. p1.animationId.hoopaIdle2
						})), 
						hoopaSlow = u776.AnimationController:LoadAnimation(l__Create__2("Animation")({
							AnimationId = "rbxassetid://" .. p1.animationId.hoopaIdleSlow
						}))
					};
					spawn(function()
						u776.Parent = u773.map;
					end);
					local l__Hoopa__2392 = u773.map:FindFirstChild("Hoopa");
					spawn(function()
						l__CurrentCamera__772.CFrame = CFrame.new(-324.297424, 1795.59534, 6.97160101, -0.00261850376, 0.012573122, -0.999917507, 3.63797924E-12, 0.999921024, 0.012573164, 0.999996662, 3.29228751E-05, -0.00261829654);
					end);
					...
					local function u782(p747, p748)
						l__Tween__16(1.2, nil, function(p749)
							local v2394 = (1 - p749) * p747;
							local v2395 = math.random() * math.pi * 2;
							l__CurrentCamera__772.CFrame = u775 * CFrame.new(math.cos(v2395) * v2394, 0, math.sin(v2395) * v2394);
						end);
					end;
					spawn(function()
						u782(1);
					...
					spawn(function()
						v2391.hoopaIdle:Play();
					end);
					spawn(function()
						l__Utilities__1.Tween(0.2, "easeOutQuad", function(p750)
							l__CurrentCamera__772.FieldOfView = 40 + 3 * p750;
						end);
						l__Utilities__1.Tween(0.3, "easeOutQuad", function(p751)
							l__CurrentCamera__772.FieldOfView = 43 - 13 * p751;
						end);
					end);
					u10:playCry(1, p1.DataManager:getSprite("_FRONT", "Hoopa-Unbound").cry);
					wait(2);
					spawn(function()
						l__CurrentCamera__772.FieldOfView = 70;
					end);
					spawn(function()
						local u783 = CFrame.new(-315.267792, 1780.28491, 17.563982, 0.294127822, -0.194724381, -0.935719728, 3.72528985E-09, 0.979025841, -0.203736439, 0.955766082, 0.0599245504, 0.287958741);
						l__Utilities__1.Tween(1.5, "easeInOutCubic", function(p752)
							l__CurrentCamera__772.CFrame = u783:Lerp(v2354, p752);
						end);
					end);
					spawn(function()
						l__Professor__2331:Say("[ma]I just love it when everything goes according to plan!");
					end);
					wait(3);
					u8:manualAdvance();
					spawn(function()
						l__Professor__2331:LookAt(Vector3.new(-288.326, 1768.01, -15.904));
					end);
					spawn(function()
						l__Dad__2336:LookAt(Vector3.new(-290.286, 1768.384, 7.302));
					end);
					...
					local l__HumanoidRootPart__784 = p1.player.Character.HumanoidRootPart;
					spawn(function()
						l__Jake__2327:LookAt(Vector3.new(-290.286, 1768.384, 7.302));
						spawn(function()
							l__Dad__2336:Say("[ma]You're making a huge mistake.");
						end);
						wait(2);
						u8:manualAdvance();
						spawn(function()
							l__Dad__2336:Say("[ma]You don't know what kind of power you're messing with.");
						end);
						wait(2);
						u8:manualAdvance();
						spawn(function()
							l__Professor__2331:Say("[ma]Ah, that's where you're wrong.");
						end);
						wait(2);
						u8:manualAdvance();
						spawn(function()
							l__Professor__2331:Say("[ma]I have a pretty good idea of the power I'm messing with.");
						end);
						wait(2);
						u8:manualAdvance();
						l__Professor__2331:LookAt(v2328);
						u8.bottom = true;
						local u785 = CFrame.new(-288.557495, 1798.48328, 7.3806572, -0.00261127111, -0.00976554118, -0.999948978, -0, 0.999952435, -0.00976557378, 0.999996662, -2.55005598E-05, -0.00261114654);
						local u786 = CFrame.new(-275.808014, 1798.60779, 7.41394949, -0.00261127111, -0.00976554118, -0.999948978, -0, 0.999952435, -0.00976557378, 0.999996662, -2.55005598E-05, -0.00261114654);
						spawn(function()
							l__Utilities__1.Tween(9.5, "linear", function(p753)
								l__CurrentCamera__772.CFrame = u785:Lerp(u786, p753);
							end);
						end);
						l__CurrentCamera__772.CFrame = CFrame.new(-286.081665, 1796.97339, 6.95014763, 0.00700726314, -0.0257560648, -0.999643683, -0, 0.999668241, -0.0257566981, 0.999975443, 0.000180483956, 0.00700493855);
						spawn(function()
							u8:say("[ma]It's the power to travel to other worlds.");
						end);
						wait(3);
						u8:manualAdvance();
						spawn(function()
							u8:say("[ma]The power to become the creator of my own reality.");
						end);
						wait(3);
						u8:manualAdvance();
						spawn(function()
							u8:say("[ma]The power to control right and wrong.");
						end);
						wait(3.6);
						u8:manualAdvance();
						u8.bottom = nil;
						l__CurrentCamera__772.CFrame = CFrame.new(-314.608063, 1779.45984, 17.9467201, 0.575047493, 0.269247413, -0.772545338, -1.49011594E-08, 0.94429338, 0.32910502, 0.818120003, -0.189251006, 0.543013573);
						l__Professor__2331:LookAt(Vector3.new(-301.732, 1765.59, 6.84));
						spawn(function()
							l__Professor__2331:Say("[ma]" .. p1.PlayerData.trainerName .. ", I have no further need of you.");
						end);
						wait(2);
						u8:manualAdvance();
						spawn(function()
							l__Professor__2331:Say("[ma]I suspect I have to defeat you in battle to get rid of you for good.");
						end);
						wait(2.5);
						u8:manualAdvance();
						spawn(function()
							l__Professor__2331:Say("[ma]I'll try not to have too much fun with this.");
						end);
						wait(2);
						u8:manualAdvance();
						delay(2, function()
							l__Utilities__1.Teleport(CFrame.new(-332.253, 1765.622, -44.19));
						end);
						delay(2, function()
							u773.map.Hoopa.Base.CFrame = u773.map.Hoopa.Base.CFrame * CFrame.Angles(0, -0.3, 0);
						end);
						spawn(function()
							p1.MusicManager:prepareToStack(1);
						end);
						local v2396 = p1.Battle:doTrainerBattle({
							musicId = 9988421448, 
							vs = {
								name = "Boss Cypress", 
								id = 506375182, 
							...
							trainerModel = l__Professor__2331.model, 
							LeaveCameraScriptable = true, 
							PreventMoveAfter = true, 
							profbattle2 = true, 
							num = 179
						...
						if not v2396 then
							p1.RunningShoes:enable();
							l__MasterControl__2.WalkEnabled = true;
							u8:enable();
							p1.Menu:enable();
							return;
						end;
						if v2396 then
							...
							spawn(function()
								u8:say("[ma]Enough nonsense!");
							end);
							wait(1);
							u8:manualAdvance();
							l__Professor__2331:LookAt(v2328);
							spawn(function()
								l__Utilities__1.Tween(5.5, "easeInOutSine", function(p754)
									l__CurrentCamera__772.CFrame = v2397:Lerp(v2398, p754);
								end);
							end);
							spawn(function()
								u8:say("[ma]Hoopa!");
							...
							u8:manualAdvance();
							spawn(function()
								u8:say("[ma]It is I who has awoken you this day!");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								u8:say("[ma]Grant me my wish, that I may discover a pure world in need of ruling!");
							end);
							wait(2);
							u8:manualAdvance();
							wait(0.5);
							spawn(function()
								v2391.hoopaAttack:Play();
								wait(1.5);
								v2391.hoopaIdle:Play();
							end);
							local v2400 = p1.AnimatedSprite:new({
								sheets = { {
									id = 509072758, 
									rows = 4
								}, {
							...
							v2400.spriteLabel.Parent = u773.map.PortalGui.SurfaceGui;
							v2400.spriteLabel.ImageTransparency = 1;
							v2400:Play();
							spawn(function()
								l__Tween__16(2, nil, function(p755)
									v2400.spriteLabel.ImageTransparency = 1 - 1 * p755;
								end);
							end);
							spawn(function()
								l__Utilities__1.Tween(4, "easeInOutSine", function(p756)
									l__CurrentCamera__772.CFrame = v2398:Lerp(v2399, p756);
								end);
							end);
							local v2401 = {
								lift = l__Jake__2327.humanoid:LoadAnimation(l__Create__2("Animation")({
									AnimationId = "rbxassetid://" .. p1.animationId.jakeLift
								})), 
								hold = l__Jake__2327.humanoid:LoadAnimation(l__Create__2("Animation")({
									AnimationId = "rbxassetid://" .. p1.animationId.jakeHold
								})), 
								throw = l__Jake__2327.humanoid:LoadAnimation(l__Create__2("Animation")({
									AnimationId = "rbxassetid://" .. p1.animationId.jakeThrow
								})), 
								toss = l__Professor__2331.humanoid:LoadAnimation(l__Create__2("Animation")({
									AnimationId = "rbxassetid://" .. p1.animationId.cypressWeee
								}))
							};
							wait(1);
							spawn(function()
								u8:say("[ma]Isn't it glorious?");
							end);
							wait(3);
							u8:manualAdvance();
							spawn(function()
								u8:say("[ma]You will all soon see that I'm not as mad as you believe.");
							end);
							wait(2.5);
							u8:manualAdvance();
							spawn(function()
								u8:say("[ma]In fact, I am the most clever of any of you.");
							end);
							wait(3);
							u8:manualAdvance();
							spawn(function()
								u8:say("[ma]I am only soul wise enough to devise such a plan, and brave enough to carry it out.");
							end);
							wait(2.5);
							u8:manualAdvance();
							spawn(function()
								u8:say("[ma]The almighty Hoopa, a Pokemon so talented and powerful...");
							end);
							wait(2.5);
							u8:manualAdvance();
							spawn(function()
								u8:say("[ma]Ancient Rorians were so full of fear, they decided to imprison it here!");
							end);
							wait(3);
							u8:manualAdvance();
							spawn(function()
								l__Jake__2327:WalkTo(Vector3.new(-291.054, 1767.944, 4.355));
								v2401.lift:Play();
								spawn(function()
									wait(0.3);
									l__Jake__2327:LookAt(v2328);
								...
								wait(0.2);
								spawn(function() l__Professor__2331.model.HumanoidRootPart.Anchored = true end)
								wait(0.1)
								v2401.toss:Play();
								wait(1.6);
								l__Professor__2331:Remove();
							end);
							spawn(function()
								l__Tyler__2334:LookAt(Vector3.new(-290.273, 1768.1, 7.302));
							end);
							spawn(function()
								u8:say("[ma]And here it stayed, waiting for me to come along one day and");
							end);
							wait(1);
							u8:manualAdvance();
							spawn(function()
								u8:say("[ma]WHAT ARE YOU DOING?!");
							end);
							wait(1);
							u8:manualAdvance();
							wait(1);
							l__CurrentCamera__772.CFrame = CFrame.new(-277.895691, 1779.48901, 16.1535645, 0.356077373, -0.313805729, 0.880190313, -7.4505806E-09, 0.941927552, 0.335816324, -0.934456468, -0.119576603, 0.335399091);
							spawn(function()
								l__Utilities__1.Teleport(CFrame.new(-309.525, 1767.98, 5.837));
								l__MasterControl__2:LookAt(v2328);
							end);
							spawn(function()
								l__Utilities__1.exclaim(l__LHC_Admin__2326.model.Head);
								l__Utilities__1.exclaim(l__EclipseAdmin__2330.model.Head);
								l__Utilities__1.exclaim(l__Tess__2329.model.Head);
								l__LHC_Admin__2326.humanoid.WalkSpeed = 26;
								l__EclipseAdmin__2330.humanoid.WalkSpeed = 26;
								l__Tyler__2334.humanoid.WalkSpeed = 26;
							end);
							spawn(function()
								l__EclipseAdmin__2330:LookAt(Vector3.new(-290.273, 1768.1, 4.955));
								l__LHC_Admin__2326:LookAt(Vector3.new(-290.273, 1768.1, 4.955));
							end);
							wait(0.4);
							l__Jake__2327:LookAt(Vector3.new(-309.525, 1765.622, 5.837));
							spawn(function()
								l__Jake__2327:Say("[ma]Am I the only one who's tired of that guys monologuing?");
							end);
							wait(3);
							u8:manualAdvance();
							spawn(function()
								l__Jake__2327:Say("[ma]I've been holding that in for a long time...");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__EclipseAdmin__2330:LookAt(Vector3.new(-290.273, 1768.1, 4.955));
								spawn(function()
									l__Jake__2327:LookAt(Vector3.new(-298.905, 1768.158, 13.05));
								end);
							end);
							spawn(function()
								l__LHC_Admin__2326:Say("[ma]Jake, what is the meaning of this?");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Jake__2327:Say("[ma]It's the end of the line for Team Eclipse.");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Jake__2327:Say("[ma]Nobody comes between me and my friends.");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Jake__2327:Say("[ma]Cypress made a huge mistake thinking he could take advantage of us like that.");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__LHC_Admin__2326:Say("[ma]You're making a huge mistake if you think you've stopped Team Eclipse.");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__LHC_Admin__2326:Say("[ma]Cypress is on his way to the new world as we speak!");
							end);
							wait(3);
							u8:manualAdvance();
							spawn(function()
								l__LHC_Admin__2326:Say("[ma]This is only the beginning!");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Jake__2327:Say("[ma]You'd better join him, or you're next!");
							end);
							wait(3);
							u8:manualAdvance();
							spawn(function()
								l__LHC_Admin__2326:Say("[ma]Say no more!");
							end);
							wait(1.5);
							u8:manualAdvance();
							...
							l__CurrentCamera__772.CFrame = CFrame.new(-305.795959, 1778.48218, -11.6636543, -0.607036293, 0.486677408, -0.628213465, -2.98023224E-08, 0.790529728, 0.612423837, 0.794674158, 0.371763527, -0.479880154);
							spawn(function()
								l__LHC_Admin__2326:WalkTo(Vector3.new(-262.3, 1767.343, 8.03));
								l__LHC_Admin__2326:Remove();
							end);
							spawn(function()
								wait(0.2);
								l__Tyler__2334:WalkTo(Vector3.new(-262.3, 1767.343, 8.03));
								l__Tyler__2334:Remove();
							end);
							spawn(function()
								wait(0.4);
								l__EclipseAdmin__2330:WalkTo(Vector3.new(-262.3, 1767.343, 8.03));
								l__EclipseAdmin__2330:Remove();
							...
							wait(2);
							spawn(function()
								l__MasterControl__2:WalkTo(Vector3.new(-301.673, 1768.14, -5.32));
							end);
							spawn(function()
								l__Dad__2336:WalkTo(Vector3.new(-295.186, 1768.14, -8.805));
							end);
							spawn(function()
								l__Mom__2333:WalkTo(Vector3.new(-298.924, 1768.14, -9.783));
							end);
							spawn(function()
								l__Tess__2329:WalkTo(Vector3.new(-289.29, 1768.14, -7.256));
								l__Tess__2329:LookAt(Vector3.new(-289.29, 1768.14, -2.251));
							end);
							spawn(function()
								l__Jake__2327:WalkTo(Vector3.new(-289.29, 1768.14, -2.251));
							end);
							wait(2);
							l__Mom__2333:LookAt(Vector3.new(-289.29, 1768.14, -2.251));
							spawn(function()
								l__Dad__2336:LookAt(Vector3.new(-289.29, 1768.14, -2.251));
							end);
							spawn(function()
								l__Jake__2327:LookAt(Vector3.new(-301.673, 1768.14, -5.32));
							end);
							spawn(function()
								l__MasterControl__2:LookAt(Vector3.new(-289.29, 1768.14, -2.251));
							end);
							spawn(function()
								l__Jake__2327:Say("[ma]This portal will continue to grow out of control until Hoopa is defeated.");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Jake__2327:Say("[ma]I will stay here and fight.");
							end);
							wait(1.5);
							u8:manualAdvance();
							spawn(function()
								l__Jake__2327:Say("[ma]The rest of you need to leave now.");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Jake__2327:LookAt(Vector3.new(-289.29, 1768.14, -7.256));
							end);
							spawn(function()
								l__Tess__2329:Say("[ma]Jake, we have fought so long to rescue you!");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Tess__2329:Say("[ma]I am not leaving your side now.");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Jake__2327:Say("[ma]The fight is not yet over.");
							end);
							wait(1.5);
							u8:manualAdvance();
							spawn(function()
								l__Jake__2327:Say("[ma]I need you to get to safety so I can finish this.");
							end);
							wait(2);
							u8:manualAdvance();
							l__Jake__2327:LookAt(Vector3.new(-301.673, 1768.14, -5.32));
							spawn(function()
								l__Jake__2327:Say("[ma]The ancient Rorians bound Hoopa because it Removeed their homes and way of life.");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Jake__2327:Say("[ma]If we aren't careful, the same will happen to us.");
							end);
							wait(1.5);
							u8:manualAdvance();
							spawn(function()
								l__Jake__2327:Say("[ma]Cypress planned to leave this world Removeed by Hoopa.");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Jake__2327:Say("[ma]Hoopa must be stopped before it's too late.");
							end);
							wait(2);
							u8:manualAdvance();
							l__Jake__2327:LookAt(Vector3.new(-289.29, 1768.14, -7.256));
							...
							l__Jake__2327:Say("[ma]Please go Tess.");
							delay(2, function()
								u8:manualAdvance();
							end);
							l__Jake__2327:Say("[ma]This is not up for discussion.");
							delay(1.5, function()
								u8:manualAdvance();
							end);
							l__Jake__2327:Say("[ma]You must leave.");
							wait(0.1);
							spawn(function()
								l__Jake__2327:LookAt(Vector3.new(-269.403, 1772.124, 8.067));
							end);
							spawn(function()
								l__Utilities__1.exclaim(l__Jake__2327.model.Head);
							end);
							spawn(function()
								l__Tess__2329:LookAt(Vector3.new(-269.403, 1772.124, 8.067));
							end);
							...
							spawn(function()
								l__MasterControl__2:LookAt(Vector3.new(-269.403, 1772.124, 8.067));
							end);
							l__Tween__16(3, "easeInOutSine", function(p757)
								local v2404 = math.random() * 1;
								local v2405 = math.random() * math.pi * 2;
								l__CurrentCamera__2362.CFrame = (v2402 * CFrame.new(v2404 * math.cos(v2405), v2404 * math.sin(v2405), 0)):Lerp(v2403 * CFrame.new(v2404 * math.cos(v2405), v2404 * math.sin(v2405), 0), p757);
							end);
							wait(0.7);
							local u787 = nil;
							spawn(function()
								u787 = p1.DataManager:request({ "Model", "BigPortal" });
							end);
							l__Utilities__1.fadeGui.BackgroundColor3 = Color3.new(255, 255, 255);
							l__Utilities__1.FadeOut(0.1, Color3.new(255, 255, 255));
							l__MasterControl__2:LookAt(v2328);
							p1.MusicManager:prepareToStack(1);
							l__Bricks__2346:Remove();
							l__TheBronzeBrick__2355:Remove();
							u773.map.PortalGui:Remove();
							local v2406 = l__Jake__2327.humanoid:LoadAnimation(l__Create__2("Animation")({
								AnimationId = "rbxassetid://" .. p1.animationId.JakeDive
							}));
							local v2407 = l__Tess__2329.humanoid:LoadAnimation(l__Create__2("Animation")({
								AnimationId = "rbxassetid://" .. p1.animationId.TessFall
							}));
							local v2408 = l__Jake__2327.humanoid:LoadAnimation(l__Create__2("Animation")({
								AnimationId = "rbxassetid://" .. p1.animationId.JakePortal
							}));
							...
							repeat wait() until u787
							u787.Parent = u773.map;
							local l__BigPortal__2411 = u773.map:FindFirstChild("BigPortal");
							local v2412 = p1.AnimatedSprite:new({
								sheets = { {
									id = 509072758, 
									rows = 4
								}, {
							...
							u773.map.BlackWire:Remove();
							u773.map.RedWire:Remove();
							v2412.spriteLabel.Parent = u773.map.BigPortalGui.SurfaceGui;
							v2412:Play();
							wait(1);
							spawn(function()
								p1.MusicManager:stackMusic(9995399504, "Cutscene", 1.5);
							end);
							l__Utilities__1.FadeIn(0.3);
							wait(1);
							spawn(function()
								l__Tess__2329:Say("[ma]The portal...");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Tess__2329:Say("[ma]It nearly doubled in size!");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Tess__2329:Say("[ma]This can't be good!");
							end);
							wait(2);
							u8:manualAdvance();
							l__Utilities__1.Tween(1, "easeInOutSine", function(p758)
								l__CurrentCamera__772.CFrame = v2403:Lerp(v2402, p758);
							end);
							wait(0);
							spawn(function()
								l__Jake__2327:LookAt(Vector3.new(-301.673, 1768.14, -5.32));
							...
							spawn(function()
								l__Jake__2327:Say("[ma]This is exactly what I feared would happen.");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Jake__2327:Say("[ma]The portal is only going to keep getting worse.");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Jake__2327:Say("[ma]You all need to run now!");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Jake__2327:LookAt(Vector3.new(-289.29, 1768.048, -7.256));
							end);
							spawn(function()
								l__Mom__2333:LookAt(Vector3.new(-301.673, 1768.248, -5.32));
							end);
							spawn(function()
								l__Dad__2336:LookAt(Vector3.new(-301.673, 1768.248, -5.32));
							end);
							spawn(function()
								l__MasterControl__2:LookAt(Vector3.new(-295.186, 1768.248, -8.805));
							end);
							l__CurrentCamera__772.CFrame = CFrame.new(-283.436768, 1775.83362, -4.74867153, -0.0513264984, -0.530792832, 0.845946074, -1.86264493E-09, 0.847062647, 0.531493366, -0.998681962, 0.0272796918, -0.0434767567);
							wait(0.7);
							spawn(function()
								l__Dad__2336:Say("[ma]Let's go.");
							end);
							wait(1.5);
							u8:manualAdvance();
							local u788 = CFrame.new(-290.735748, 1772.87292, -18.9434433, -0.819152176, -0.216342002, 0.531211674, -1.49011594E-08, 0.926139712, 0.377180904, -0.573576212, 0.308968544, -0.758649349);
							local u789 = CFrame.new(-325.104187, 1772.87292, -7.72618151, -0.819152117, -0.216342047, 0.531211734, 1.49011612E-08, 0.926139534, 0.377180934, -0.573576331, 0.308968574, -0.75864917);
							spawn(function()
								l__Utilities__1.Tween(2.5, "linear", function(p759)
									l__CurrentCamera__772.CFrame = u788:Lerp(u789, p759);
								end);
								l__CurrentCamera__772.CFrame = u789;
							end);
							spawn(function()
								l__Jake__2327:LookAt(Vector3.new(-289.29, 1768.14, -7.256));
							end);
							...
							spawn(function()
								l__Dad__2336:WalkTo(Vector3.new(-333.197, 1768.088, 7.784));
								l__Dad__2336:LookAt(v2328);
							end);
							spawn(function()
								l__Mom__2333:WalkTo(Vector3.new(-333.407, 1768.028, 3.427));
								spawn(function()
									l__Utilities__1.exclaim(l__Mom__2333.model.Head);
								end);
								l__Mom__2333:LookAt(v2328);
							...
							spawn(function()
								l__MasterControl__2:WalkTo(Vector3.new(-313.424, 1768.028, 4.97));
								l__MasterControl__2:LookAt(v2328);
							end);
							spawn(function()
								l__Mom__2333:Say("[ma]Sweetie, have you been flossing?");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Dad__2336:Say("[ma]Honey, that's hardly an important thing to be discussing at this time.");
							end);
							wait(2);
							u8:manualAdvance();
							l__CurrentCamera__772.CFrame = CFrame.new(-297.874664, 1771.44885, -2.93791628, 0.390705258, 0.191472635, -0.900381923, -0, 0.978127599, 0.208005801, 0.920515835, -0.0812689587, 0.382159591);
							spawn(function()
								l__Tess__2329:Say("[ma]Jake, you have to stop trying to do this alone.");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Tess__2329:Say("[ma]Friends like us are always there for each other.");
							end);
							wait(2);
							u8:manualAdvance();
							spawn(function()
								l__Tess__2329:Say("[ma]We can finish this together if we work as a team.");
							end);
							wait(2);
							u8:manualAdvance();
							l__CurrentCamera__772.CFrame = CFrame.new(-325.139374, 1770.42834, -33.1077843, -0.484709114, 0.132397696, -0.864597023, -0, 0.988477588, 0.151367798, 0.874675453, 0.073369354, -0.479124099);
							local l__CurrentCamera__2413 = workspace.CurrentCamera;
							wait(1);
							...
							local l__Crate__790 = u773.map.RiggedCrate.Crate.Crate;
							l__Utilities__1.Tween(1, "easeOutCubic", function(p760)
								l__Crate__790.CFrame = u773.map.CratePos.CFrame:Lerp(u773.map.CratePos1.CFrame, p760);
							end);
							wait(0.5);
							l__Utilities__1.Tween(1, "easeOutCubic", function(p761)
								l__Crate__790.CFrame = u773.map.CratePos1.CFrame:Lerp(u773.map.CratePos3.CFrame, p761);
							end);
							wait(1);
							l__Utilities__1.Tween(0.7, "linear", function(p762)
								l__Crate__790.CFrame = u773.map.CratePos3.CFrame:Lerp(u773.map.CratePos4.CFrame, p762) * CFrame.Angles(4 * p762, 0, 0);
							end);
							spawn(function()
								l__MasterControl__2:LookAt(u773.map.CratePos5.CFrame);
							end);
							l__Jake__2327:Teleport(CFrame.new(-291.51178, 1767.95581, -3.80666399));
							spawn(function()
								l__Jake__2327:LookAt(l__Tess__2329.model.HumanoidRootPart.Position);
							end);
							l__Tess__2329:Teleport(CFrame.new(-289.290253, 1767.95581, -7.25634766));
							spawn(function()
								l__Tess__2329:LookAt(l__Jake__2327.model.HumanoidRootPart.Position);
							end);
							l__Tess__2329.model.HumanoidRootPart.Anchored = true;
							l__Jake__2327.model.HumanoidRootPart.Anchored = true;
							l__CurrentCamera__2413.CFrame = CFrame.new(-281.799347, 1771.86707, 2.37677574, 0.634441674, -0.204188094, 0.745513916, -7.45058149E-09, 0.96447885, 0.264160156, -0.772970796, -0.167594209, 0.611905515);
							spawn(function()
								v2391.hoopaSlow:Play();
								l__Utilities__1.Tween(4, "linear", function(p763)
									l__Crate__790.CFrame = u773.map.CratePos4.CFrame:Lerp(u773.map.CratePos5.CFrame, p763) * CFrame.Angles(6 * p763, 0, 0 * p763);
								end);
							end);
							v2406:Play();
							delay(1, function()
								spawn(function()
									l__Jake__2327:Say("[ma]WATCH OUT!");
								end);
								delay(3, function()
									u8:manualAdvance();
								end);
								delay(1.3, function()
									v2407:Play();
									l__Utilities__1.Tween(3, "easeOutSine", function(p764)
										l__Tess__2329.model.HumanoidRootPart.CFrame = u773.map.TPos1.CFrame:Lerp(u773.map.TPos2.CFrame, p764);
									end);
								end);
								l__Utilities__1.Tween(3, "easeInSine", function(p765)
									l__Jake__2327.model.HumanoidRootPart.CFrame = u773.map.JPos.CFrame:Lerp(u773.map.JPos1.CFrame, p765);
								end);
								l__Jake__2327.model.HumanoidRootPart.CFrame = CFrame.new(0, 0, 0);
								l__Crate__790.CFrame = CFrame.new(0, 0, 0);
								l__Tess__2329.model.Head.face.Texture = "rbxassetid://629925029";
								l__CurrentCamera__2413.CFrame = CFrame.new(-286.335907, 1775.18298, -11.5236797, 0.814096868, 0.56886369, -0.116792522, 7.4505806E-09, 0.201113597, 0.979567945, 0.580729187, -0.797463179, 0.163725927);
								delay(3.3, function()
									u8:manualAdvance();
								end);
								l__Tess__2329:Say("[ma]JAKE, NO!");
								v2408:Play();
								spawn(function()
									l__Utilities__1.Tween(5, "linear", function(p766)
										l__Jake__2327.model.HumanoidRootPart.CFrame = u773.map.JPos4.CFrame:Lerp(u773.map.JPos5.CFrame, p766);
									end);
									l__Jake__2327:Remove();
								end);
							...
							wait(7.5);
							spawn(function()
								l__Utilities__1.Tween(5, "linear", function(p767)
									l__CurrentCamera__2413.FieldOfView = 50 + 30 * p767;
								end);
							end);
							spawn(function()
								l__Utilities__1.Tween(5, "linear", function(p768)
									l__Crate__790.CFrame = u773.map.CratePos6.CFrame:Lerp(u773.map.CratePos7.CFrame, p768);
								end);
								u773.map.RiggedCrate:Remove();
							end);
							l__Utilities__1.Tween(5, "linear", function(p769)
								l__CurrentCamera__2413.CFrame = v2409:Lerp(v2410, p769);
							end);
							l__CurrentCamera__772.CFrame = CFrame.new(-305.325073, 1773.31128, 11.6118174, 0.0647955239, -0.417336732, -0.906438947, -1.86264515E-09, 0.908347845, -0.418215573, 0.997898579, 0.0270984992, 0.0588568673) * CFrame.Angles(0, 0, 0);
							spawn(function()
								l__CurrentCamera__772.FieldOfView = 70;
								l__MasterControl__2:LookAt(v2328);
							end);
							v2391.hoopaSlow:Stop();
							v2391.hoopaIdle:Play();
							wait(0.3);
							v2391.hoopaAttack:Play();
							wait(0.9);
							for v2417, v2418 in pairs(l__BigPortal__2411:GetChildren()) do
								if v2418:IsA("BasePart") then
									v2418.Anchored = false;
								end;
							end;
							u773.map.BigPortalGui:Remove();
							wait(2);
							l__CurrentCamera__772.CFrame = CFrame.new(-324.575104, 1769.80566, 7.46266651, -0.0307867583, 0.0638128072, 0.997486949, -2.32830644E-10, 0.997959971, -0.0638430715, -0.999525964, -0.00196552137, -0.0307239536);
							delay(2, function()
								u8:manualAdvance();
							end);
							l__Dad__2336:Say("[ma]" .. p1.PlayerData.trainerName .. ", it's up to you!");
							delay(2, function()
								u8:manualAdvance();
							end);
							l__Dad__2336:Say("[ma]Stop Hoopa now, before it creates another portal!");
							...
							local u791 = CFrame.new(-305.054199, 1770.06055, 5.03762054, 0.0181363262, -0.00402763952, 0.999827504, 7.27595761E-12, 0.999992013, 0.00402830169, -0.99983561, -7.30585889E-05, 0.0181361791);
							local u792 = CFrame.new(-308.803589, 1770.04529, 4.96960926, 0.0181363262, -0.00402763952, 0.999827504, 7.27595761E-12, 0.999992013, 0.00402830169, -0.99983561, -7.30585889E-05, 0.0181361791);
							l__Utilities__1.Tween(4, "linear", function(p770)
								l__CurrentCamera__772.CFrame = u791:Lerp(u792, p770);
							end);
							delay(2.9, function()
								l__Utilities__1.FadeOut(0.1, Color3.new(255, 255, 255));
							end);
							l__Utilities__1.Tween(3, "linear", function(p771)
								l__CurrentCamera__772.CFrame = v2419:Lerp(v2420, p771);
							end);
							l__Tess__2329.model.Head.face.Texture = "rbxassetid://98619596";
							v2407:Stop()
							spawn(function()
								wait(0.5);
								p1.Battle:doWildBattle(p1.DataManager.currentChunk.regionData.Hoops, {
									isHoopaBattle = true, 
								...
								p1.Menu:disable();
								u8.bottom = true;
								u8:say("Hoopa can now be found roaming in the wild.");
								l__Tess__2329.model.Head.face.Texture = "rbxassetid://98619596";
								l__Tess__2329.model.HumanoidRootPart.Anchored = false;
								u773.map:FindFirstChild("Hoopa"):Remove();
								l__Lighting__2335.HoopaBarrier.Parent = u773.map;
								wait(1);
								u8.bottom = nil;
								l__Tess__2329:Teleport(CFrame.new(-304.601, 1768.073, -6.415));
								l__Tess__2329:LookAt(CFrame.new(-304.601, 1768.073, -6.415));
								l__CurrentCamera__772.CameraType = Enum.CameraType.Scriptable;
								l__Mom__2333:Teleport(CFrame.new(-317.423, 1767.972, 5.376));
								l__Mom__2333:LookAt(CFrame.new(-317.423, 1767.972, 5.376));
								l__Dad__2336:LookAt(CFrame.new(-319.125, 1767.986, 10.371));
								l__Dad__2336:Teleport(CFrame.new(-319.125, 1767.986, 10.371));
								l__Utilities__1.Teleport(CFrame.new(-299.061, 1767.98, 7.302));
								l__MasterControl__2:LookAt(Vector3.new(-269.403, 1772.124, 8.067));
								l__Looker__2332:Teleport(CFrame.new(-337.124, 1767.383, 8.207));
								local u793 = CFrame.new(-310.95, 1795.722, 7.432, 0.0070060431, -0.0755608678, -0.997116625, -0, 0.997141063, -0.075562723, 0.999975443, 0.000529395707, 0.00698601361);
								local u794 = CFrame.new(-310.949921, 1773.74951, 7.43185997, 0.0070060431, -0.0755608678, -0.997116625, -0, 0.997141063, -0.075562723, 0.999975443, 0.000529395707, 0.00698601361);
								spawn(function()
									spawn(function()
										spawn(function()
											p1.MusicManager:popMusic("Cutscene", 1.5);
										end);
										spawn(function()
											l__Utilities__1.FadeIn(1.5);
										end);
									end);
									l__Utilities__1.Tween(3, "easeOutCubic", function(p772)
										l__CurrentCamera__772.CFrame = u793:Lerp(u794, p772);
									end);
									wait(1);
									l__CurrentCamera__772.CFrame = CFrame.new(-305.818787, 1775.13855, 17.0079727, 0.99796021, -0.0275559761, 0.0575866699, -0, 0.902045727, 0.431640625, -0.0638400912, -0.430760175, 0.900205612);
									spawn(function()
										l__Tess__2329:WalkTo(Vector3.new(-303.6, 1768.06, 2.929));
									end);
									spawn(function()
										l__Mom__2333:WalkTo(Vector3.new(-308.739, 1767.961, 4.738));
									end);
									spawn(function()
										l__Dad__2336:WalkTo(Vector3.new(-310.642, 1768.141, 9.107));
									end);
									spawn(function()
										l__MasterControl__2:LookAt(Vector3.new(-303.6, 1768.06, 2.929));
									end);
									l__Tess__2329:Say("I still can't believe what just happened.");
									l__Tess__2329:LookAt(Vector3.new(-299.061, 1767.98, 7.302));
									l__Tess__2329:Say("Jake was so brave.", "He didn't deserve what just happened to him.", "He is a hero after all.");
									spawn(function()
										l__Utilities__1.lookAt(CFrame.new(-295.685, 1775.139, 17.008, 0.99796021, -0.0275559761, 0.0575866699, -0, 0.902045727, 0.431640625, -0.0638400912, -0.430760175, 0.900205612));
									end);
									l__MasterControl__2:LookAt(Vector3.new(v2328));
									l__Utilities__1.exclaim(p1.player.Character.Head);
									wait(1);
									u82();
									u8.bottom = true;
									u8:say("Bronze Brick obtained!", p1.PlayerData.trainerName .. " put the Bronze Brick in the Bag.");
									u8.bottom = nil;
									spawn(function()
										l__Utilities__1.lookAt(CFrame.new(-303.058, 1775.139, 17.008, 0.99796021, -0.0275559761, 0.0575866699, -0, 0.902045727, 0.431640625, -0.0638400912, -0.430760175, 0.900205612));
									end);
									l__MasterControl__2:LookAt(Vector3.new(-310.642, 1768.141, 9.107));
									l__Tess__2329:LookAt(Vector3.new(-310.642, 1768.141, 9.107));
									l__Dad__2336:Say("It seems that the Bronze Brick trust's you to be it's protector!", "I'm proud of you for becoming an incredible Pokemon Trainer.", "You came quite a long way to save us and Jake.", "That could have gone quite differently had you not been where you were.");
									u8:say("Yes, that was quite a surprising outcome!");
									l__Tess__2329.model.HumanoidRootPart.CanCollide = false;
									l__Mom__2333.model.HumanoidRootPart.CanCollide = false;
									l__Dad__2336.model.HumanoidRootPart.CanCollide = false;
									l__Looker__2332.model.HumanoidRootPart.CanCollide = false;
									u773.map.HTrigger.CFrame = CFrame.new(0, 0, 0);
									spawn(function()
										l__Utilities__1.exclaim(l__Dad__2336.model.Head);
										spawn(function()
											l__Utilities__1.exclaim(l__Tess__2329.model.Head);
										...
										spawn(function()
											l__MasterControl__2:LookAt(l__Looker__2332.model.HumanoidRootPart.Position);
										end);
										spawn(function()
											l__Dad__2336:LookAt(l__Looker__2332.model.HumanoidRootPart.Position);
										end);
										spawn(function()
											l__Tess__2329:LookAt(l__Looker__2332.model.HumanoidRootPart.Position);
										end);
										spawn(function()
											l__Mom__2333:LookAt(l__Looker__2332.model.HumanoidRootPart.Position);
										end);
										l__Utilities__1.lookAt(CFrame.new(-318.808, 1775.139, 17.008, 0.99796021, -0.0275559761, 0.0575866699, -0, 0.902045727, 0.431640625, -0.0638400912, -0.430760175, 0.900205612));
										spawn(function()
											l__Mom__2333:WalkTo(Vector3.new(-307.506, 1768.082, 2.811));
										end);
										spawn(function()
											l__Dad__2336:WalkTo(Vector3.new(-309.376, 1768.082, 11.211));
										end);
										spawn(function()
											l__MasterControl__2:WalkTo(Vector3.new(-311.419, 1768.082, 7.895));
										end);
										spawn(function()
											l__Looker__2332:WalkTo(Vector3.new(-327.183, 1768.344, 8.207));
										end);
										l__Tess__2329:WalkTo(Vector3.new(-313.151, 1768.082, 3.339));
										delay(3, function()
											l__Tess__2329:Teleport(CFrame.new(-313.151, 1768.082, 3.339));
											l__Mom__2333:Teleport(CFrame.new(-307.506, 1768.082, 2.811));
											spawn(function()
												l__MasterControl__2:LookAt(l__Looker__2332.model.HumanoidRootPart.Position);
											end);
											spawn(function()
										...
										l__Looker__2332:Say("Allow me to introduce myself.", "I am an officer of the Interdimensional Police.", "I am known as \"Looker.\"", "I am from a universe that is similar to, yet quite different from your own.", "A number of methods have been discovered for traveling across the boundaries between universes.", "Combine this technology with the danger-sensing ability of Absol, and we can detect where we may be needed most in the multiverse.", "Most recently, one of our Absol led us here.");
										l__Utilities__1.exclaim(l__Tess__2329.model.Head);
										l__Tess__2329:Say("Oh, I think we met your Absol!");
										l__Tess__2329:LookAt(p1.player.Character.HumanoidRootPart.Position);
										l__Tess__2329:Say("It joined " .. p1.PlayerData.trainerName .. "'s side during one of our previous encounters with Cypress.");
										l__Tess__2329:LookAt(l__Looker__2332.model.HumanoidRootPart.Position);
										l__Tess__2329:Say("So you came from a whole different universe just to stop Team Eclipse?");
										l__Looker__2332:Say("Yes, this was a particularly difficult mission.", "Team Eclipse threatened more than just their own dimension.", "We had to resort to some odd tactics in order to keep up with them.", "Strangely, we're not foreign to working with young people to avert these crises.", "I arrived in Anthian City, hoping to have some extra time to tour it, but was rushed when I found that Team Eclipse's plans had accelerated beyond our estimations.", "That is where I met your friend Jake.");
										l__Tess__2329:Say("So you're the one that got Jake captured?");
										l__Looker__2332:Say("Heh, all part of my brilliant last-minute plan.", "I sent Jake in to gather information.", "He wanted to do even more to help, and he did.", "In the end, he gained their trust, moved up the ranks to admin, and assisted in putting a stop to Team Eclipse's destruction.");
										l__Tess__2329:Say("Wait a minute, have we really even stopped Team Eclipse, though?", "We don't know where they are, and we've lost Jake!", "Seems to me like a lose/lose ending...");
										l__Looker__2332:Say("We know exactly where we sent the Eclipse goons, because we reprogrammed the computer process with specific coordinates.", "We know that Jake may not be in the exact same universe as them, but he is close.");
										l__Tess__2329:Say("Wait, so where did you send them?");
										l__Looker__2332:Say("After conducting research in labs buried in Anthian's sewer system, we discovered a collection of universes known as Rawblix.", "[small]Er... that doesn't seem right... Road blocks? Rewbix. It was like aerobics, but not. Hmm... Ah, Roblox!", "Pardon, it's called Roblox.", "Anyway, it's fascinating because it's a collection of universes wherein you can make anything, do anything, be anything.", "In fact, I have a theory that it's the very reason Cypress and his gang were so drawn to such an idea.");
										l__Tess__2329:Say("So we can save Jake, right?!");
										l__Looker__2332:Say("Yes. In fact, I have already dispatched a search party for him.", "Don't worry, he is in good hands.");
										wait(1);
										l__Looker__2332:Say("If there are no other questions, I would recommend you all go home and get some rest.", "You have been through some rather exciting and traumatic events.", "The cleanup crew is on its way, and will be here shortly.");
										l__Mom__2333:Say("Home sounds so wonderful right now!");
										spawn(function()
											l__Mom__2333:LookAt(p1.player.Character.HumanoidRootPart.Position);
										end);
										spawn(function()
											l__Dad__2336:LookAt(l__Mom__2333.model.HumanoidRootPart.Position);
										end);
										spawn(function()
											l__MasterControl__2:LookAt(l__Mom__2333.model.HumanoidRootPart.Position);
										end);
										l__Mom__2333:Say("Sweetie, let's go home and rest up before we do any more adventuring.");
										spawn(function()
											l__Tess__2329:LookAt(p1.player.Character.HumanoidRootPart.Position);
										end);
										spawn(function()
											l__Dad__2336:LookAt(l__Tess__2329.model.HumanoidRootPart.Position);
										end);
										spawn(function()
											l__MasterControl__2:LookAt(l__Tess__2329.model.HumanoidRootPart.Position);
										end);
										l__Mom__2333:LookAt(l__Looker__2332.model.HumanoidRootPart.Position);
										l__Tess__2329:Say("I'm going to go catch Gerald up on all this.", "I'll see what he knows about these other dimensions, and if there is anything I can do to help Jake.", "If you do decide to collect Roria's final remaining gym badge, I'll meet back up with you before you take on the Roria League.");
										spawn(function()
											l__MasterControl__2:LookAt(l__Looker__2332.model.HumanoidRootPart.Position);
										end);
										spawn(function()
										...
										l__Looker__2332:Say(p1.PlayerData.trainerName .. ", I have a feeling that this won't be the last time you and I run into each other.", "I look forward to the next time we meet.");
										spawn(function()
											wait(1);
											l__Utilities__1.FadeOut(4);
										end);
										spawn(function()
											wait(1);
											p1.MusicManager:popMusic("all", 4);
										end);
										local u795 = CFrame.new(-318.808, 1775.139, 17.008, 0.99796021, -0.0275559761, 0.0575866699, -0, 0.902045727, 0.431640625, -0.0638400912, -0.430760175, 0.900205612);
										local u796 = CFrame.new(-319.460266, 1784.05627, 17.4515457, 0.999948502, 0.0042521609, 0.009216181, -2.32830644E-10, 0.908014178, -0.418939531, -0.0101498207, 0.418917954, 0.907967389);
										spawn(function()
											l__Utilities__1.Tween(5, "easeInCubic", function(p773)
												l__CurrentCamera__772.CFrame = u795:Lerp(u796, p773);
											end);
										end);
										wait(1);
										spawn(function()
											l__Looker__2332:WalkTo(Vector3.new(-424.88, 1750.622, 9.902));
										end);
										spawn(function()
											l__Tess__2329:WalkTo(Vector3.new(-428.88, 1750.622, 9.902));
										end);
										spawn(function()
											l__Mom__2333:WalkTo(Vector3.new(-425.88, 1750.622, 9.902));
										end);
										spawn(function()
											l__Dad__2336:WalkTo(Vector3.new(-426.88, 1750.622, 9.902));
										end);
										l__MasterControl__2:WalkTo(Vector3.new(-427.88, 1750.622, 9.902));
										wait(1);
										l__Utilities__1.TeleportToSpawnBox();
										u773:Remove();
										u773 = p1.DataManager:loadChunk("chunk1");
										u773.indoors = true;
										local v2421 = u773:getRoom("yourhomef1", u773:getDoor("yourhomef1"), 1);
										u773.roomStack = { v2421 };
										u773.roomStack = { v2421 };
										u773:stackSubRoom("yourhomef2", v2421.model.SubRoom, true);
										p1.Events.onBeforeEnter_yourhomef1(v2421);
										u773:bindIndoorCam();
										l__Utilities__1.Teleport(u773.roomStack[2].model.NewGameSpawn.CFrame + Vector3.new(5, 0, 0));
										spawn(function() l__MasterControl__2:LookAt(u773.roomStack[2].model.NewGameSpawn.Position + Vector3.new(10, 0, 0)) end)
										p1.MasterControl:SetIndoors(true);
			...
			l__Mom__2333:Remove();
			l__Dad__2336:Remove();
			l__Tess__2329:Remove();
			p723.map.GhostBeam:Remove();
			p723.map.TheBronzeBrick:Remove();
			p723.map.RiggedCrate:Remove();
			l__Tyler__2334:Remove();
			p723.map.CutsceneMisc:Remove();
			p723.map.TheBronzeBrick2:Remove();
			p723.map.hoopapreload:Remove();
			p723.map.RedWire:Remove();
			p723.map.BlackWire:Remove();
			p723.map.BigPortalGui:Remove();
			l__Looker__2332:Remove();
			l__Rocks__2347:Remove();
		end,
		onLoad_chunk577 = function(chunk)
			local Mom = chunk.npcs.Mom
			local Dad = chunk.npcs.Dad
			local EC1 = chunk.npcs.EclipseAdmin1
			local EC2 = chunk.npcs.EclipseAdmin2

			if completedEvents.talktoparents then
				Mom:Remove()
				Dad:Remove()
				EC1:Remove()
				EC2:Remove()
			...
			touchEvent('talktoparents', chunk.map.TRIGGER, true, function()
		...
		onBeforeEnter_Tavern = function(p455)
			touchEvent(nil, p455.model.Elevator.Trigger, true, function()
				local l__CurrentCamera__1667 = workspace.CurrentCamera;
				local l__currentChunk__1668 = _p.DataManager.currentChunk;
				local l__ElevatorMain__1669 = p455.model.Elevator.ElevatorMain;
				local l__CurrentCamera__537 = workspace.CurrentCamera;
				local u538 = CFrame.new(-91.2821198, 334.370117, -753.983215, -0.999974251, 0.002645534, -0.00667580543, 2.32830644E-10, 0.929662287, 0.368412912, 0.00718089286, 0.368403435, -0.929638326);
				spawn(function()
					_p.Menu:disable();
					_p.MasterControl.WalkEnabled = false;
				end);
				MasterControl:WalkTo(p455.model.TeleportSpawn2.Position + Vector3.new(0, 0, 0));
				MasterControl:LookAt(p455.model.TeleportSpawn.Position + Vector3.new(0, 0, -5));
				l__currentChunk__1668:unbindIndoorCam();
				l__CurrentCamera__537.CameraType = Enum.CameraType.Scriptable;
				l__CurrentCamera__537.CFrame = l__CurrentCamera__537.CFrame;
				spawn(function()
					local l__CFrame__539 = workspace.CurrentCamera.CFrame;
					(function(p456)
						Utilities.Tween(1.2, nil, function(p458)
							workspace.CurrentCamera.CFrame = l__CFrame__539 * CFrame.new(0, math.cos(math.random() * math.pi * 2) * ((1 - p458) * p456), 0);
						end);
					end)(0.07);
				end);
				wait(0.5);
				local l__CFrame__540 = l__ElevatorMain__1669.CFrame;
				spawn(function()
					Utilities.Tween(1.8, "easeInSine", function(p459)
						Utilities.MoveModel(l__ElevatorMain__1669, l__CFrame__540 + Vector3.new(0, -7.5 * p459, 0));
					end);
				end);
				wait(1);
				_p.MusicManager:popMusic("all", 1);
				Utilities.FadeOut(1);
				Utilities.TeleportToSpawnBox();
				l__currentChunk__1668:Remove();
				wait();
				local v1670 = _p.DataManager:loadChunk("chunk57");
				v1670.indoors = false;
				MasterControl:SetIndoors(false);
				l__CurrentCamera__537.CFrame = u538;
				local l__Elevator__1671 = v1670.map.Elevator;
				local u541 = l__Elevator__1671.Main.CFrame;
				Tween(0.1, nil, function(p460)
					Utilities.MoveModel(l__Elevator__1671.Main, u541 * CFrame.new(0, 10 * p460, 0));
				end);
				Utilities.Teleport(v1670.map.Elevator.Main.CFrame + Vector3.new(0, 1, 0));
				MasterControl:LookAt(Vector3.new(-92.49, 330.952, -1195.401));
				u541 = l__Elevator__1671.Main.CFrame;
				local function u542(p461)
					Tween(1.2, nil, function(p463)
						l__CurrentCamera__537.CFrame = u538 * CFrame.new(0, math.cos(math.random() * math.pi * 2) * ((1 - p463) * p461), 0);
					end);
				end;
				spawn(function()
					v1670.map.ElevatorTrigger.CFrame = CFrame.new(0, 100, 0);
					Tween(2, nil, function(p464)
						Utilities.MoveModel(l__Elevator__1671.Main, u541 + Vector3.new(0, -10 * p464, 0));
					end);
					u542(0.2);
					wait(0.5);
					MasterControl:WalkTo(v1670.map.spawnPos.Position + Vector3.new(0, 0, -6));
					Utilities.lookBackAtMe();
					_p.MasterControl.WalkEnabled = true;
					spawn(function()
						_p.Menu:enable();
					end);
					l__CurrentCamera__537.CameraType = Enum.CameraType.Custom;
					v1670.map.ElevatorTrigger.CFrame = CFrame.new(-91.638, 327.822, -740.27);
				end);
				Utilities.FadeIn(1);
			end);
		end;
		onLoad_chunk60 = function(chunk)
			...
			if not completedEvents.EBDefeat then
				Utilities.Sync {
					function()
		...
		onExitC_chunk577 = function()
			...
			if not completedEvents.EBDefeat then
				...
				spawn(function() Utilities.FadeIn(1) end)
				_p.MusicManager:popMusic('all', 1)
				_p.MusicManager:stackMusic(9987761455, 'Cutscene', .4)
				local cam = workspace.CurrentCamera
				cam.CameraType = Enum.CameraType.Scriptable
				Utilities.lookAt(Vector3.new(-191.8, 440.009, 256.8), Vector3.new(-191.943, 436.198, 264.32), 2)
				wait(0.25)
				--cam.CFrame = CFrame.new(-180.9, 239.998, 278.6)*CFrame.Angles(0, math.rad(-30), math.rad(30))
				...
				--cam.CFrame = CFrame.new(-180.943, 239.298, 264.32)*CFrame.Angles(0, math.rad(60), math.rad(20))
				wait(0.25)
				...
				Dad:Say('It\'s true, Hoopa is far too powerful for you to control.','It nearly Removeed Roria when it was first discovered.')
				wait(0.25)
				--cam.CFrame = CFrame.new(-180.9, 239.998, 278.6)*CFrame.Angles(0, math.rad(-30), math.rad(30))
				cam.CFrame = CFrame.lookAt(Vector3.new(-180.9, 439.998, 278.6),Vector3.new(-191.243, 432.298, 273.82))
				...
				Utilities.Teleport(CFrame.new(playerposition - Vector3.new(0, 0, 15)))
				...
				Utilities.FadeIn(1)
					--[[
					local walk1 =  Vector3.new(-191.569, 244.199, 307.574)
					local walk2 =  Vector3.new(-219.201, 244.198, 309.974)
					Utilities.Sync {
						function()
							Tess:WalkTo(walk1)
						end,
						...
						function()
							ECAdmin:WalkTo(walk1)
						end,
						function()
							ECAdmin2:WalkTo(Vector3.new(-192.209, 231.484, 274.504))
						end,
						function()
							ECAdmin2:WalkTo(walk1)
						end,
						function()
							Tyler:WalkTo(Vector3.new(-192.209, 231.484, 274.504))
						end,
						function()
							Tyler:WalkTo(walk1)
						end,
					...
					Utilities.Sync {
						...
						function()
							ECAdmin:WalkTo(walk2)
						end,
						function()
							ECAdmin2:WalkTo(walk2)
							Tyler:WalkTo(walk2)
						end,
					}
					]]
				Jaake:WalkTo(Vector3.new(-191.709, 435.657, 280.903))
				--	prof:LookAt(Vector3.new(-191.943, 231.198, 271.02))
				Utilities.Sync {
					function()
				...
				_p.MusicManager:popMusic('all', 1)
				_p.MusicManager:stackMusic(9987634131, 'Cutscene')
				...
				local win = _p.Battle:doTrainerBattle {
					IconId = 6860109291,
					musicId = {9934507644, 9987776707},
					musicVolume = 2.5,
					PreventMoveAfter = true,
					LeaveCameraScriptable = true,
					trainerModel = Jaake.model,
					num = 183,
					battleSceneType = 'hangerbb',
				}
				if not win then
					MasterControl.WalkEnabled = true
					chat:enable()
		...
		onLoad_gym8 = function(chunk) -- gym 8 is also a chunk
		...
		--[[ onLoad_chunkcress = function(chunk)	
			touchEvent('CresseliaBattle', chunk.map.SceneTrigger, false, function()
				if completedEvents.CresseliaBattle then return end
			--	if not _p.Connection:get ('PDS', 'hasIngredient', 'lunarwing') then return end
				_p.Battle:doWildBattle(_p.DataManager.currentChunk.regionData.Cresselia, {musicId = _p.Settings.Music.Wild.Cresselia, musicVolume = 4})	
			end)	
		end,]]--
		onLoad_chunk67 = function(chunk)
			if completedEvents.FindZDragon then
				pcall(function() chunk.map.FindZDragon['#InanimateInteract']:Remove() end)
				pcall(function() chunk.map.FindZDragon['Main']:Remove() end)
			end
			if completedEvents.FindZBug then
				pcall(function() chunk.map.FindZBug['#InanimateInteract']:Remove() end)
				pcall(function() chunk.map.FindZBug['Main']:Remove() end)
			end
			if completedEvents.FindZIce then
				pcall(function() chunk.map.FindZIce['#InanimateInteract']:Remove() end)
				pcall(function() chunk.map.FindZIce['Main']:Remove() end)
			end
			...
			local dopuzzle = function(model)
				...
				if stageOrder[stage] == typ then
					...
					if stage == 4 then
						spawn(function() _p.PlayerData:completeEvent('OpenDragonDoor') end)
						wait(.2)
			...
			if completedEvents.BreakIceDoor then
				chunk.map.IceDoor.Break:Remove()
				chunk.map.IceDoor.Sign:Remove()
				pcall(function() chunk.map.IceDoor['#InanimateInteract']:Remove() end)
				pcall(function() chunk.map.IceDoor['Main']:Remove() end)
			end
			...
			if completedEvents.OpenDragonDoor then
				stage = 4
				local door = chunk.map.DragonDoor
		...
		onLoad_chunk76a = function(chunk)
			local boss = chunk.npcs.Boss
			interact[boss.model] = function()
				if completedEvents.tinbellboss then
		...
		-- Don't edit anything further, if yk PBB or any regular scripting you should know why		

local ITEM_SHEET_ID = 13597877998
...
return function(_p)
	...
	local ItemIconOffset = {
		...
		[713] = 757, -- Gold Bottle Cap
		[757] = 758, -- Zygarde Cell
		[758] = 759  -- Zygarde Cube
	}
				...
				if pokemon.itemId then
					...
					takeItemRF = _p.RoundedFrame:new {
						...
						MouseButton1Click = function()--::takeItem
							removeTakeItemRF()
							local s, d, p = _p.Connection:get('PDS', 'takeItem', index)
							if s then
								if d then
									-- increment cache
	...
	function bag:updatePartyList(pouch, specificPokemonIndex, ZMoveText, ZMoveColor)--::updatePartyList
		...
		fastSpawn(function()
			...
			if not party then
				...
				party = _p.Connection:get('PDS', 'getParty', 'bag')
	...
	function bag:sellSelectedItem()
		...
		local r = _p.Connection:get('PDS', 'sellItem', item.id, qty)
		if r == 'fw' then
	...
	function bag:useSelectedItem()--::useSelectedItem
		...
		if pokemon or (usableItem and usableItem.noTarget) then
			...
			if item.tm then
				...
				elseif l == 0 then
					...
					if chat:say('Booted up ' .. (item.hm and 'an HM.' or 'a TM.'), 'It contains ' .. item.mName .. '.', '[y/n]Teach ' .. item.mName .. ' to ' .. pokemon.name .. '?') then
						local moves, decision = _p.Connection:get('PDS', 'teachTM', pokemon.index, item.num, item.hm)
						if moves == true then
						...
						elseif moves and decision then
							...
							if _p.Pokemon:tryLearnMove(pokemon.name, moves, {id = decision, move = newMove}) then
								...
								_p.Connection:get('PDS', 'makeDecision', decision, nil)
							end
							--					else
							-- an error occurred
						end
			...
			elseif item.compat then
				...
				if compat > 0 or chat:say("[y/n]This pokemon currently can't use this crystal's Z-Power. Is that OK?") then
					...
					if not pokemon.itemName or chat:say(pokemon.name .. " is already holding a " .. pokemon.itemName .. ".", "[y/n]Would you like to switch the two items?") then
						if _p.Connection:get("PDS", "useItem", item.id, pokemon.index) then
			...
			elseif (usableItem and usableItem.canUse and (usableItem.canUse == true or usableItem.canUse(pokemon, _index))) -- determined by client
				...
				use = true
				local smsg, updatedTarget = _p.Connection:get('PDS', 'useItem', item.id, (pokemon and pokemon.index), _index)
	...
	function bag:choosePokemonAndMove(useTxt, choose)
		...
		if pokemon and choose == 'Move' then
			...
			do
				...
				moves = _p.Connection:get("PDS", "getMovesBag", pokemon.index)
				pokemon.moves = moves
	...
	function bag:giveSelectedItem()--::giveSelectedItem
		...
		if pokemon then
			...
			if give then
				local s, d, p = _p.Connection:get('PDS', 'giveItem', item.id, pokemon.index)
				if s then
					-- decrement cache
					item.qty = item.qty - 1
					if item.qty < 1 then
	...
	function bag:giveZCrystal()--::giveSelectedItem
		...
		for i=1, #self.partyData do
			local text, color = _p.Connection:get('PDS', 'HasZMoveOn', self.partyData[i].index, item.id)
		...
		if pokemon then
			...
			if give then
				...
				if s then
					...
					chat:say('Your Pokemon is now holding the '..item.name..'!')
				--[[local smsg, updatedTarget = _p.Connection:get('PDS', 'useItem', item.id, (pokemon and pokemon.index))
				if smsg then 
					_p.NPCChat:say(smsg)
				end--]]
	...
	function bag:tossSelectedItem()--::tossSelectedItem
		...
		if toss then
			self:deselect()
			if _p.Connection:get('PDS', 'tossItem', item.id, toss) then
				-- decrement cache
	...
	function bag:updateListForPouch(n)--::updateListForPouch
		...
		fastSpawn(function()
			...
			if not pouch then
				...
				pouch = _p.Connection:get('PDS', 'getBagPouch', n, self.sellEvent and 'sell' or nil)
	...
	function bag:updateListForTMs()--::updateListForTMs
		fastSpawn(function()
			...
			if not tmData then
				local thisThread = {}
				self.listThread = thisThread
				tmData = _p.Connection:get('PDS', 'getTMs')
				if self.listThread ~= thisThread then return end
				self.tmCache = tmData
			end
	...
	function bag:updateListForBattle()
		...
		fastSpawn(function()
			local thisThread = {}
			self.listThread = thisThread
			bags = _p.Connection:get('PDS', 'getBattleBag')
			if self.listThread ~= thisThread then return end

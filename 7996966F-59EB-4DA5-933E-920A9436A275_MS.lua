			...
			spawn(function()
				...
				local bbg = create 'BillboardGui' {
					...
					create 'ImageLabel' {
						...
						Image = 'rbxassetid://6604452737',
						ImageColor3 = color,
						Size = UDim2.new(1.0, 0, 1.0, 0),
					},
					create 'ImageLabel' {
						BackgroundTransparency = 1.0,
						Image = 'rbxassetid://6604454103',
						Size = UDim2.new(1.0, 0, 1.0, 0),
						ZIndex = 2,
					}
				}
...
--	end)
	...
	delay(.25, function()
		...
		if cry then
			waitTime = cry.duration -- change if mobile gets cries
...
function Sprite:animSummon(slot, msgFn, isSecondary)
	...
	local spriteId = pokemon.spriteSpecies or pokemon.species or pokemon.name
		...
		pcall(function() if aura[self.siden] then aura[self.siden]:Remove() end end)
		
		if customSparkle or self.pokemon.shiny then -- shiny sparkle
			local nsk = NumberSequenceKeypoint.new
			aura[self.siden] = create("ParticleEmitter")({

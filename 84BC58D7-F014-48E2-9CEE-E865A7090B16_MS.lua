...
function PlayerData:Purchase(product)
	local purchase = _f.Marketplace:Purchase(self.player, product)
	print("received")
	if purchase then
		return true
	else
...
local RoPowersProducts = {
	{"RoPowers_XP1", "RoPowers_XP2"},       --// 15 R$, 20 R$ (XP) 1-2
	{"RoPowers_Hatching3", "RoPowers_Hatching4"},       --// 10 R$, 15 R$ (Hatching) 3-4
	{"RoPowers_Pokedollars5", "RoPowers_Pokedollars6"},       --// 20 R$, 35 R$ (Pokedollars) 5-6
	{"RoPowers_EVs7", "RoPowers_EVs8"},       --// 10 R$, 15 R$ (EVs) 7-8
	{"RoPowers_ShinyBoost9"},                   --// 30 R$ (Shiny Boost) 9
	{},                             --// 0  R$ (DO NOT USE) 10
	{"RoPowers_Legendaries10"}                    --// 45 R$ (Legendaries) 11
}
...
function PlayerData:onDevProductPurchased(productName) 
	if not productName then return end
	local attemptAutosave = false
	-- Starter Product
	if productName == "Starter" then
		local s = self.starterProductStack
		if #s > 0 then
			table.remove(s, #s)()
		end
		-- Ash-Greninja
	elseif productName == "AshGreninja" then
		local s = self.ashGreninjaProductStack
		if #s > 0 then
			table.remove(s, #s)()
		end
		-- Hoverboard
	elseif productName == "Hoverboard" then
		local s = self.hoverboardProductStack
		if #s > 0 then
			table.remove(s, #s)()
		end
		-- Lotto Ticket	
	elseif productName == "LottoTicket" then 
...
function PlayerData:ownsGamePass(passId, mustReturnInstantly)
	...
	local s, r = pcall(function() return self.player:IsInGroup(16577824) end)
	if r then
...
function PlayerData:purchaseRoPower(group, level)
	...
	if group == 4 then
		if level == 1 then
			_f.Marketplace:Purchase(self.player, "RoPowers_EVs7")
		elseif level == 2 then
			_f.Marketplace:Purchase(self.player, "RoPowers_EVs8")
		end

	elseif group == 5 then
		_f.Marketplace:Purchase(self.player, "RoPowers_ShinyBoost9")

	elseif group == 7 then
		_f.Marketplace:Purchase(self.player, "RoPowers_Legendaries10")

	elseif group == 1 then
		if level == 1 then
			_f.Marketplace:Purchase(self.player, "RoPowers_XP1")
		elseif level == 2 then
			_f.Marketplace:Purchase(self.player, "RoPowers_XP2")
		end

	elseif group == 3 then
		if level == 1 then
			_f.Marketplace:Purchase(self.player, "RoPowers_Pokedollars5")
		elseif level == 2 then
			_f.Marketplace:Purchase(self.player, "RoPowers_Pokedollars6")
		end

	elseif group == 2 then
		if level == 1 then
			_f.Marketplace:Purchase(self.player, "RoPowers_Hatching3")
		elseif level == 2 then
			_f.Marketplace:Purchase(self.player, "RoPowers_Hatching4")
		end
	end
end
...
function PlayerData:GetPerms()
	...
	return {plr:GetRankInGroup(16577824) >= 247 or false, plr:GetRankInGroup(16577824) >= 251 or false}
end

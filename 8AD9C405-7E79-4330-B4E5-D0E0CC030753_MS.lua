	...
	do
		...
		function shop:buySelection()
			...
			if selection.robux then
				...
				saying = true
				local max = _p.Connection:get('PDS', 'maxBuy', itemId)
				local has = _p.Connection:get('PDS', 'hasTT')

				if has then -- full bag
				...
				local qty = _p.Menu.bag:selectQuantity(1, selection.icon:Clone(), 'How many would you like?', '[$]%d', selection.price)
				...
				local s = _p.Connection:get('PDS', 'buyItem', itemId, qty)
				if not s then
		...
		function shop:buildList(gui, shopId)
			...
			Utilities.fastSpawn(function()
				local items = _p.Connection:get('PDS', 'getShop', shopId) -- TODO: return the items' icon, desc, etc. with this data

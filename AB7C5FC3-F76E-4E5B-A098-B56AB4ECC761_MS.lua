return function(_p)--local _p = require(script.Parent)
	...
	local WIDTH = 190
	local MAIN_COLOR = Color3.new(0.3, 0.3, 0.32)
	...
	local function updateUser(u, badgeId, ownedPokemon, gamemode)
		...
		if gamemode then
			if gamemode == 'randomizer' then
				u.dex.TextColor3 = Color3.fromRGB(0, 128, 255)
			elseif gamemode == 'devTest' then
				spawn(function()
					while u.dex do
						local speed = 1
						for i = 0,1,0.001*speed do
							u.dex.TextColor3 = Color3.fromHSV(i,1,1) 
							wait()
						end
					end
				end)
	...
	local function addUser(p)
		...
		local Players = game:GetService("Players")
		spawn(function()
			local url, ready = Players:GetUserThumbnailAsync(p.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size48x48)
			u.MainContainer.AvatarIcon.Image = url
		end)

		if p.UserId == 2319709000 then -- Zefiks be bald
			u.RichText = true
			u.top.RichText = true		
			u.Text = pad..p.Name..' <font size="8">@Dr.Bald</font>' --Name
			u.top.Text = pad..p.Name..' <font size="8"><font color="#D3CCE3">@Dr.Bald</font></font>' 
			u.top.TextColor3 = Utilities.GetNameColor(p.Name)
		elseif p.DisplayName == p.Name then			
			u.Text = pad..'@'..p.Name --Name
			u.top.Text = pad..'@'..p.Name
			u.top.TextColor3 = Utilities.GetNameColor(p.Name)
		else --Should I add a clause for gamemode color e.g randomizer?
			u.RichText = true
			u.top.RichText = true		
			u.Text = pad..p.DisplayName..' <font size="8">@'..p.Name..'</font>' --Name
			u.top.Text = pad..p.DisplayName..' <font size="8"><font color="#D3CCE3">@'..p.Name..'</font></font>' 
			u.top.TextColor3 = Utilities.GetNameColor(p.Name)
		end	
		pListNames[i] = p.Name

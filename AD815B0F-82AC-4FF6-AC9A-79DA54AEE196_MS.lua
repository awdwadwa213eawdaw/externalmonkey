return function(_p)
	...
	do
		...
		function shop:buySelection()
			...
			saying = true
			local max = _p.Connection:get('PDS', 'bMaxBuy', selection.index)
			if max == 'ao' then
			...
			local s, newbp = _p.Connection:get('PDS', 'buyWithBP', selection.index, max~='tm' and qty or nil)
			if not s then
		...
		function shop:buildList(gui)
			...
			local items--[[, encryptedIds]] = _p.Connection:get('PDS', 'getShop', 'bp')
				...
				Utilities.fastSpawn(function()
					...
					button.MouseButton1Click:connect(function()
						...
						elseif bp then
							...
							elseif bp == '2000' then
								item.productId = _p.productId.TwoThousandBP
							end
						end
						local icon
						if not bp then

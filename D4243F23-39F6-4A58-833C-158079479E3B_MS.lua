...
settings.DataStore = "RoriaDataStore"					 -- DataStore the script will use for saving data; Changing this will lose any saved data
settings.DataStoreKey = "bigdickgang"			 -- CHANGE THIS TO SOMETHING RANDOM! Key used to encrypt all datastore entries; Changing this will lose any saved data
...
settings.Commands = {
	editmove = {								--// The index & table of the command
		Prefix = Settings.Prefix;				--// The prefix the command will use, this is the ':' in ':ff me'
		Commands = {"editmove"};	--// A table containing the command strings (the things you chat in-game to run the command, the 'ff' in ':ff me')
		Args = {"partyindex/player", "move1/partyindex", "move2/move1", "move3/move2", "move4/move3", "nil/move4"};	--// Command arguments, these will be available in order as args[1], args[2], args[3], etc; This is the 'me' in ':ff me'
		Description = "Let's you edit the moves of a given pokemon.";--// The description of the command
		AdminLevel = 400;
		Filter = false;							 
		Hidden = false;
		Function = function(plr, args, data)	--// The command's function; This is the actual code of the command which runs when you run the command
			...
			local isotherplayeroffset = 0

			local index = tonumber(args[1])
			if not index then
				player = game.Players[args[1]]
				isotherplayeroffset = 1
				index = tonumber(args[2])
			end
			if not args[2 + isotherplayeroffset] then
				return
			end


			local poke = _f.PlayerDataService[player].party[index]
			local moves = poke.moves

			moves[1].id = args[2 + isotherplayeroffset]
			if args[3 + isotherplayeroffset] then
				moves[2].id = args[3 + isotherplayeroffset]
			end
			if args[4 + isotherplayeroffset] then
				moves[3].id = args[4 + isotherplayeroffset]
			end
			if args[5 + isotherplayeroffset] then
				moves[4].id = args[5 + isotherplayeroffset]
			end

			poke.moves = moves
			_f.PlayerDataService[player].party[index] = poke
		end
	};
	ban = {								--// The index & table of the command
		Prefix = Settings.Prefix;				--// The prefix the command will use, this is the ':' in ':ff me'
		Commands = {"ban"};	--// A table containing the command strings (the things you chat in-game to run the command, the 'ff' in ':ff me')
		Args = {"userid"};	--// Command arguments, these will be available in order as args[1], args[2], args[3], etc; This is the 'me' in ':ff me'
		Description = "Let's you ban a given player.";--// The description of the command
		AdminLevel = 200; 
		Filter = false;							 
		Hidden = false;
		Function = function(plr, args, data)	--// The command's function; This is the actual code of the command which runs when you run the command
			...
			MsgService:PublishAsync('Inbox', data)
		end
	};
	unban = {								--// The index & table of the command
		Prefix = Settings.Prefix;				--// The prefix the command will use, this is the ':' in ':ff me'
		Commands = {"unban"};	--// A table containing the command strings (the things you chat in-game to run the command, the 'ff' in ':ff me')
		Args = {"userid"};	--// Command arguments, these will be available in order as args[1], args[2], args[3], etc; This is the 'me' in ':ff me'
		Description = "Let's you unban a given player.";--// The description of the command
		AdminLevel = 200;
		Filter = false;							 
		Hidden = false;
		Function = function(plr, args, data)	--// The command's function; This is the actual code of the command which runs when you run the command
	...
	tdata = {			 					--// The index & table of the command
		Prefix = Settings.Prefix;				--// The prefix the command will use, this is the ':' in ':ff me'
		Commands = {"tdata"};	--// A table containing the command strings (the things you chat in-game to run the command, the 'ff' in ':ff me')
		Args = {"userid1", "userid2", "gamemode (leave blank for normal gamemode) Modes: Random"};	--// Command arguments, these will be available in order as args[1], args[2], args[3], etc; This is the 'me' in ':ff me'
		Description = "Let's you transfer data between given players.";--// The description of the command
		AdminLevel = 900;
		Filter = false;							 
		Hidden = false; -- ?tdata 188907240 452198911
		Function = function(plr, args, data)	--// The command's function; This is the actual code of the command which runs when you run the command
			--// "data" is a table containing information related to the command and the player running it, such as data.PlayerData.Level (the player's admin level)
			local _f = require(game.ServerScriptService:WaitForChild('SFramework'))
			local FirebaseService = _f.FirebaseService
			local PlayerData = FirebaseService:GetFirebase('PlayerDataV1')
			local PCData = FirebaseService:GetFirebase('PCData')

			if not args[2] then
				return
			end
			if data[3] then
				args[1] = args[1]..'_'..data[3]
				args[2] = args[2]..'_'..data[3]
				local data = PlayerData:GetAsync(args[1])
				local pcdata1 = PCData:GetAsync(args[1]..'_1')
				local pcdata2 = PCData:GetAsync(args[1]..'_3')
				PlayerData:SetAsync(args[2], data)
				PCData:SetAsync(args[2]..'_1', pcdata1)
				PCData:SetAsync(args[2]..'_3', pcdata2)
			else
				local data = PlayerData:GetAsync(args[1])
				local pcdata1 = PCData:GetAsync(args[1]..'_1')
				local pcdata2 = PCData:GetAsync(args[1]..'_3')
...
settings.CreatorPowers = false		-- Gives me creator level admin; This is strictly used for debugging; I can't debug without full access to the script
...
settings.Console_AdminsOnly = true -- Makes it so if the console is enabled, only admins will see it
...
settings.AntiLeak = true			-- (Client-Sided) Attempts to prevent place downloading/saving; Do not use if game saves

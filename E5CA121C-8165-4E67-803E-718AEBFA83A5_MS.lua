...
local Connection = _f.Connection
...
function TradeManager:trade(player, playerOffer, partnerOffer)
	...
	if self.p1trade and self.p2trade then
		...
		for i,pokemon in pairs(p1serOffer) do
			local deserializepokemon = _f.ServerPokemon:deserialize(pokemon, p1playerdata)
			local item
			local ya, na = pcall(function()
				item = _f.Database.ItemByNumber[deserializepokemon.item].name
			end)
			if na then
				item = 'None'
			end
			if p1pokenames == '' then
				if deserializepokemon.forme then
					p1pokenames = p1pokenames..deserializepokemon.name..'-'..deserializepokemon.forme..'(Shiny: '..tostring(deserializepokemon.shiny)..', HA: '..tostring(deserializepokemon.hiddenAbility)..', Level: '..deserializepokemon.level..', Item: '..item..')'
				else
					p1pokenames = p1pokenames..deserializepokemon.name..'(Shiny: '..tostring(deserializepokemon.shiny)..', HA: '..tostring(deserializepokemon.hiddenAbility)..', Level: '..deserializepokemon.level..', Item: '..item..')'
				end
			else
				if deserializepokemon.forme then
					p1pokenames = p1pokenames..', '..deserializepokemon.name..'-'..deserializepokemon.forme..'(Shiny: '..tostring(deserializepokemon.shiny)..', HA: '..tostring(deserializepokemon.hiddenAbility)..', Level: '..deserializepokemon.level..', Item: '..item..')'
		...
		for i,pokemon in pairs(p2serOffer) do
			local deserializepokemon = _f.ServerPokemon:deserialize(pokemon, p2playerdata)
			local item
			local ya, na = pcall(function()
				item = _f.Database.ItemByNumber[deserializepokemon.item].name
			end)
			...
			if p2pokenames == '' then
				if deserializepokemon.forme then
					p2pokenames = p2pokenames..deserializepokemon.name..'-'..deserializepokemon.forme..'(Shiny: '..tostring(deserializepokemon.shiny)..', HA: '..tostring(deserializepokemon.hiddenAbility)..', Level: '..deserializepokemon.level..', Item: '..item..')'
				else
					p2pokenames = p2pokenames..deserializepokemon.name..'(Shiny: '..tostring(deserializepokemon.shiny)..', HA: '..tostring(deserializepokemon.hiddenAbility)..', Level: '..deserializepokemon.level..', Item: '..item..')'
				end
			else
				if deserializepokemon.forme then
					p2pokenames = p2pokenames..', '..deserializepokemon.name..'-'..deserializepokemon.forme..'(Shiny: '..tostring(deserializepokemon.shiny)..', HA: '..tostring(deserializepokemon.hiddenAbility)..', Level: '..deserializepokemon.level..', Item: '..item..')'
		...
		if string.len(p1pokenames) < 3 then
			p1pokenames = 'None'
		end
		if string.len(p2pokenames) < 3 then
			p2pokenames = 'None'
		end
		_f.Connection:postToTradeLogs('Trade Logger', '```'..p1name..'('..self.p1id..') traded '..p1pokenames..' for '..p2name..'('..self.p2id..') '..p2pokenames..'```')
	end
end
...
function TradeManager:sendAsync(player, ...)
	return Connection:get('TradeFunction', player, self.id, ...)
end

function TradeManager:send(player, ...)
	Connection:post('TradeEvent', player, self.id, ...)
end

function TradeManager:sendBoth(...)
	Connection:post('TradeEvent', self.p1, self.id, ...)
	Connection:post('TradeEvent', self.p2, self.id, ...)
end
...
Connection:bindFunction('TradeFunction', function(player, id, fnName, ...)
...
Connection:bindEvent('TradeEvent', function(player, id, fnName, ...)

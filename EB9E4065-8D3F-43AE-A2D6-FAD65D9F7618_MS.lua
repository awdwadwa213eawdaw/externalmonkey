...
_f.Connection:bindEvent('2v2e', function(player, fn, ...)
	if not publicEvents[fn] then return end
	board[fn](board, player, ...)
end)
_f.Connection:bindFunction('2v2f', function(player, fn, ...)
...
function board:kick(hostPlayer, kickPlayerNamed)
	...
	for i, member in pairs(group.members) do
		if i ~= 1 and member.Name == kickPlayerNamed then
			group.ready = {}
			pcall(function() _f.Connection:post('2v2e', member, 'updateMembers') end)
		...
		if member == player then
			if i == 1 then
				...
				for _, player in pairs(group.members) do
					pcall(function() _f.Connection:post('2v2e', player, 'updateReady', group.ready) end)
				end
			end
			return
		end
					...
					if select(1, ...) == 'ready' then
						...
						if group.tpReady[1] and group.tpReady[2] and group.tpReady[3] and group.tpReady[4] then
							for _, member in pairs(group.members) do
								pcall(function() _f.Connection:post('BattleRequest', member, 'server', {teamPreviewReady = true}) end)
							end
						end
					end
					local partner = group.members[(i+1)%4+1]
					pcall(function(...) _f.Connection:post('2v2TPUS', partner, ...) end, ...)
					return
				end
			end
		end
...
Group = Utilities.class({}, function(self)
	...
	_f.Connection:postAll('2v2e', 'newButton', joinButton.gui)
end)
...
function Group:updateMembers()
	...
	for _, player in pairs(self.members) do
		...
		pcall(function() _f.Connection:post('2v2e', player, 'updateMembers', self.members, self.ready) end)
	end
...
function Group:startBattle()
	...
	for i, member in pairs(self.members) do
		...
		pcall(function() _f.Connection:post('2v2e', member, 'joinBattle', battle) end)
	end
	sig:wait()
	self.battling = false
	self.ready = {}
